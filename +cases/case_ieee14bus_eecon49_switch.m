function case_data = case_ieee14bus_eecon49_switch()
%CASE_IEEE14BUS_EECON49_SWITCH  IEEE14 operating case mapped from EECON49 Fig. 4.
%
% SOURCE_DEFINED:
%   - network, loads, voltage base and frequency: cases.case_ieee14bus;
%   - SG at bus 1; IBRs at buses 2,3,6,8;
%   - apparent powers printed in Fig. 4:
%       SG1=134.62 MVA, IBR=[36.31 33.02 50.86 30.64] MVA.
%   - GFL IBRs are PQ resources (not PV voltage controllers).
%
% PROJECT_DERIVED_FIGURE_MATCH:
%   The paper does not publish the separate P/Q dispatch values.  A common
%   positive reactive-power angle phi=0.500345721564827 rad is the positive-Q
%   root for which the in-house PF simultaneously preserves the four printed
%   IBR apparent powers and gives |S_SG1|=134.62 MVA.  The negative-Q root is
%   rejected because the paper describes IBR voltage/reactive support and the
%   original IEEE14 slack absorbs reactive power.  This is an explicit mapping
%   assumption, not a claim that the unpublished source P/Q values are known.
%
% No PF solution is loaded: bus P/Q inputs are frozen below and the project
% Newton-Raphson solver computes all voltages, angles and slack outputs.

% Start from the project-owned MATPOWER-6 data container so the reporting PF
% and the production composite DAE share one ID/base/topology contract.  The
% numerical solve still uses the in-house Newton implementation.
case_data = cases.case_matpower6_case14();

% Phase D (CASE_DEFINED): slack setpoint 1.06 -> 1.00 pu at angle 0, and a
% +/-5% admissible voltage band on every bus.  Applied here, not in
% cases.case_matpower6_case14, because that file's mpc.bus(:,8) also feeds its
% reference_solution (the MATPOWER validation baseline for other consumers).
% This case already discards that baseline (see reference_solution below).
%
% Voltage limits live in mpc.bus columns 12/13 only.  The 12-column bus_data
% contract has no Vmax/Vmin column (cols 11/12 are Qmin_pu/Qmax_pu), so the
% band is recorded in mpc and in voltage_limit_contract; it is a reporting and
% admissibility band, not a PF equation input.
ids = case_data.bus_data(:,1);
slack_idx = find(ids==1,1);
case_data.bus_data(slack_idx,3) = 1.00;   % PF slack |V| setpoint (was 1.06)
case_data.bus_data(slack_idx,4) = 0.0;    % PF slack angle reference
case_data.mpc.bus(case_data.mpc.bus(:,1)==1, 8) = 1.00;
case_data.mpc.bus(case_data.mpc.bus(:,1)==1, 9) = 0.0;
case_data.mpc.gen(case_data.mpc.gen(:,1)==1, 6) = 1.00;  % Vg setpoint of SG1
case_data.mpc.bus(:,12) = 1.05;           % Vmax, +5% about the 1.00 reference
case_data.mpc.bus(:,13) = 0.95;           % Vmin, -5% about the 1.00 reference
case_data.voltage_limit_contract = struct( ...
    'classification','CASE_DEFINED', ...
    'V_ref_pu',1.00,'band_pct',5,'Vmax_pu',1.05,'Vmin_pu',0.95, ...
    'applies_to','all buses', ...
    'role','reporting and admissibility band; not a PF equation input', ...
    'note',['Replaces the MATPOWER 1.06/0.94 band, which was +/-6% about ' ...
            'the original 1.06 slack setpoint.'], ...
    'known_violation',['Bus 6 solves at 1.057535 pu, exceeding Vmax by ' ...
            '7.5e-3 pu, at the frozen Figure-4 mapping.  Reported, not tuned: ' ...
            'the mapped IBR6 reactive dispatch and the 5-6 tap 0.932 are ' ...
            'SOURCE/CASE_DEFINED and are not adjusted to satisfy the band.']);

% Phase E: label the inverter buses by resource role.  These four are GFL
% PQ resources (grid-following current sources on the reported operating
% point; the switching study later re-roles them to GFM as the island
% reference when the SG is absent).  bus_role is a presentation/labelling
% field that standardize_case keeps parallel to the numeric 12-column
% bus_data; it never feeds the PF equations.
case_data.bus_role = repmat("PQ",size(case_data.bus_data,1),1);
case_data.bus_role(1) = "REF";
case_data.bus_role(case_data.bus_data(:,2)==2) = "PV";
ibr_buses = [2 3 6 8];
for k = 1:numel(ibr_buses)
    j = find(case_data.bus_data(:,1)==ibr_buses(k),1);
    case_data.bus_role(j) = "GFL";   % initial grid-following PQ resource
end

S_ibr_pu = [0.3631 0.3302 0.5086 0.3064];
% PROJECT_DERIVED_FIGURE_MATCH, re-derived for the Phase D slack setpoint.
% fzero on |S_SG1(phi)| - 1.3462 over the positive-Q bracket [0.30, 0.90] rad
% converged to a residual of -1.11e-15 pu; the slack remains reactive-absorbing
% (Q_SG1 = -0.195666 pu), so the original root-selection rationale is preserved.
% Superseded value at slack 1.06: 0.500345721564827 rad.
phi = 0.508383707164714;
P_ibr_pu = S_ibr_pu*cos(phi);
Q_ibr_pu = S_ibr_pu*sin(phi);

ids = case_data.bus_data(:,1);
for k = 1:numel(ibr_buses)
    j = find(ids==ibr_buses(k),1);
    case_data.bus_data(j,2) = 3;       % GFL PQ resource
    case_data.bus_data(j,3) = 1.0;     % flat-start only; solved output
    case_data.bus_data(j,4) = 0.0;
    case_data.bus_data(j,5) = P_ibr_pu(k);
    case_data.bus_data(j,6) = Q_ibr_pu(k);
    % Keep the dual representation numerically identical.  MATPOWER bus type
    % 1 denotes PQ (project bus type 3); Pg/Qg remain resource inputs in the
    % production device table, while the mpc rows preserve auditable mapping.
    jm = find(case_data.mpc.bus(:,1)==ibr_buses(k),1);
    jg = find(case_data.mpc.gen(:,1)==ibr_buses(k),1);
    case_data.mpc.bus(jm,2) = 1;
    case_data.mpc.gen(jg,2) = 100*P_ibr_pu(k);
    case_data.mpc.gen(jg,3) = 100*Q_ibr_pu(k);
end

case_data.system_name = 'IEEE 14-Bus EECON49 Figure-4 Switching Case';
case_data.reference_solution = struct(); % baseline MATPOWER solution no longer applies
case_data.reference = struct( ...
    'network','MATPOWER 6.0 case14.m (IEEE CDF), solved by project Newton PF', ...
    'sg_dynamics','EECON49 H,D with project-audited Kodsi EMF6 reactance/time-constant mapping', ...
    'sg_model','Kundur/GENTPJ operational EMF6');
case_data.eecon49_mapping = struct( ...
    'classification','PROJECT_DERIVED_FIGURE_MATCH', ...
    'source_file','docs/text/EECON49_[Nui].pdf, Figure 4', ...
    'sg_bus',1,'ibr_buses',ibr_buses, ...
    'S_ibr_pu',S_ibr_pu,'S_sg_target_pu',1.3462, ...
    'common_power_angle_rad',phi,'common_power_factor',cos(phi), ...
    'P_ibr_pu',P_ibr_pu,'Q_ibr_pu',Q_ibr_pu);

% EMF6 SG data for the dynamic EECON49 route.  The paper specifies a
% sixth-order SG but does not print its reactance/time-constant table; the
% project therefore reuses the audited IEEE14/Kodsi machine coefficients and
% converts the published H=2.5 s, D=1.0 pu system-base values to the 615-MVA
% machine base used by that source table.  This is explicitly
% PROJECT_DERIVED_SOURCE_MAPPED, not a claim that the omitted paper table is
% known.
machine_to_system = 100/615;
% Tpq0=0 is the audited Kodsi round-rotor singular limit.  The operational
% composite EMF6 route keeps the six-coordinate public order and freezes Edp
% through its documented algebraic-limit contract.
case_data.machines = struct( ...
    'model','emf6', ...
    'base',struct('S_MVA',615,'V_kV',0,'f_Hz',60), ...
    'reactances',struct('Xl',0.2396,'Ra',0.0, ...
        'Xd',0.8979,'Xdp',0.2995,'Xdpp',0.23, ...
        'Xq',0.646,'Xqp',0.646,'Xqpp',0.4), ...
    'time_constants',struct('Tpd0',7.4,'Tppd0',0.03, ...
        'Tpq0',0.0,'Tppq0',0.033), ...
    'exciter',struct('model','constant_field','KA',0,'TA',1), ...
    'units',struct('gen_id','SG1','bus',1, ...
        'H',2.5*machine_to_system,'D',1.0*machine_to_system));
case_data.eecon49_mapping.sg_model = 'Kundur/GENTPJ EMF6; H=2.5 s, D=1.0 pu on 100-MVA system base; coefficients mapped from audited Kodsi table';
case_data.eecon49_mapping.sg_model_classification = 'PROJECT_DERIVED_SOURCE_MAPPED';
% The paper does not publish a post-trip active-power allocation.  Freeze a
% deterministic project-derived schedule before dynamics: retain each mapped
% pre-trip IBR dispatch and distribute the PF-derived lost-SG active power in
% proportion to the existing CASE_DEFINED IBR nameplate proxies.  Frequency
% droop absorbs the small loss mismatch in the all-KCL equilibrium; no value is
% tuned against the transient result.
pmax_MW = struct('IBR2',140,'IBR3',100,'IBR6',100,'IBR8',100);
lost_sg_MW = 133.190444; % project PF result at the frozen mapped operating point
                         % (Phase D slack |V| = 1.00 pu; was 131.895 at 1.06)
share = [140 100 100 100]/440;
post_MW = 100*P_ibr_pu + lost_sg_MW*share;
% The same frozen operating-point accounting is required for reactive power.
% SG1 absorbs reactive power at the mapped PF point; opening it without
% reducing IBR Q_ref leaves an artificial reactive surplus and drives the
% constant-impedance TS load model to a high-voltage/current-limited branch.
% Preserve the pre-trip total generation by allocating the lost SG Q through
% the already frozen participation vector. This is a case/event input, never
% an equilibrium output fitted after simulation.
lost_sg_Q_MVAr = -19.5665559229952;
post_Q_MVAr = 100*Q_ibr_pu + lost_sg_Q_MVAr*share;
case_data.dispatch_contract = struct( ...
    'policy','mapped_pre_dispatch_plus_pmax_proportional_lost_sg', ...
    'classification','PROJECT_DERIVED_FROZEN_BEFORE_TRANSIENT', ...
    'pre_fault',struct('SG1_Pg_MW',lost_sg_MW, ...
        'IBR2_Pg_MW',100*P_ibr_pu(1),'IBR3_Pg_MW',100*P_ibr_pu(2), ...
        'IBR6_Pg_MW',100*P_ibr_pu(3),'IBR8_Pg_MW',100*P_ibr_pu(4)), ...
    'post_trip',struct('deficit_MW',lost_sg_MW, ...
        'lost_sg_Q_MVAr',lost_sg_Q_MVAr, ...
        'participation',struct('IBR2',share(1),'IBR3',share(2), ...
            'IBR6',share(3),'IBR8',share(4)), ...
        'post_trip_Pg_MW',struct('IBR2',post_MW(1),'IBR3',post_MW(2), ...
            'IBR6',post_MW(3),'IBR8',post_MW(4)), ...
        'post_trip_Qg_MVAr',struct('IBR2',post_Q_MVAr(1),'IBR3',post_Q_MVAr(2), ...
            'IBR6',post_Q_MVAr(3),'IBR8',post_Q_MVAr(4))), ...
    'pmax_MW',pmax_MW,'load_shed_MW',0);
case_data.dispatch_contract.mode_aware_post_trip=struct( ...
    'policy','forming_units_hold_pretrip_Pref; GFL_units_share_lost_SG_by_Pmax', ...
    'classification','PROJECT_DERIVED_FROZEN_BEFORE_TRANSIENT', ...
    'rationale',['A newly forming VSG retains its event-left active-power ' ...
        'reference so the mode transfer does not create an artificial swing ' ...
        'acceleration. Remaining GFL power controllers share the lost SG ' ...
        'dispatch by the frozen Pmax participation.']);
case_data.switching_event_contract = struct( ...
    'classification','CASE_DEFINED_FROM_EVENT_SEQUENCE_FIGURE', ...
    'T_end',200.0, ...
    'sg_trip_time',20.0, ...
    'step_on',50.0,'step_factor',0.20,'step_all_loads',true, ...
    'fault_on',85.0,'fault_clear',85.15,'fault_bus',9, ...
    'fault_Zf',0.01+0.01i, ...
    'line_trip_time',110.0,'line_from_bus',6,'line_to_bus',13, ...
    'restore_time',145.0, ...
    'restore_sg',true,'restore_line',true,'restore_base_loads',true);
% Reuse the already frozen project supervisory thresholds/delays.  This changes
% only the operating-point/event data case; AGSI/selector timing remains ours.
mission_contract=cases.case_ieee14_1sg_4ibr_auto_vsg();
case_data.synchronism=mission_contract.synchronism;
case_data.delays=mission_contract.delays;
case_data.selector=mission_contract.selector;
% Re-run standardize so the human-readable tables reflect the final bus_role,
% slack setpoint, and MPC edits made above (bus_role was set after the
% standardize call inside cases.case_matpower6_case14).
case_data = cases.standardize_case(case_data);
end
