function dev = decoupled_dual_mode_model(device_id,bus_id,bus_position, ...
    bus_ids,V0,params,P_ref_pu,Q_ref_pu,E_ref_pu,mode)
%DECOUPLED_DUAL_MODE_MODEL  Fixed-layout GFL/GFM IBR with a decoupled GFM swing.
%
% Same dual-mode contract as ibr.eecon49_dual_mode_model (which is retained
% unchanged as the comparison baseline), with one substitution: the GFM branch
% is ibr.gfm_decoupled_full_model, whose swing block separates droop (R),
% transient damping (D_t, via a washout state) and inertia (M) into
% independent knobs.  The GFL branch, the shared plant, the DC closure, the
% transfer contract and the input ABI are unchanged.
%
% Superset state (17, fixed):
%   common plant 1:3
%     [i_d i_q V_dc]
%   GFL controller 4:9 (PLL present)
%     [delta_PLL xi_PLL xi_P xi_Q xi_Id xi_Iq]
%   GFM controller 10:17 (NO PLL)
%     [delta_VSG omega_VSG E xi_Vd xi_Vq xi_Id xi_Iq omega_f]
%
% Active states are 9 in GFL and 11 in GFM.  omega_f is the LAST superset
% state, so indices 1:16 keep exactly the meaning they have in the 16-state
% baseline layout.  The command-delay states of the source model remain
% reduced out by singular perturbation (v_del = v_cmd, T_d << dt, defect
% TD-2026-08-12-01).
%
% Fixed input ABI:
%   u = [P_ref; Q_ref; E_ref].
...
% One addition specific to this model: on entry to GFM the washout state is
% initialized TRACKED (omega_f = omega_VSG) so the transient-damping term
% D_t*(omega - omega_f) is exactly zero at the transfer instant.  Leaving
% omega_f at its equilibrium value 1 while omega is carried over from the PLL
% would inject a step torque D_t*(omega_PLL - 1) that no source or derivation
% justifies.  omega_f does not enter the current injection, so the existing
% current-continuity gate is unaffected by this choice.
...
if ~isfinite(V0) || abs(V0)<=0
    error('ibr:decoupled_dual_mode_model:badV0', ...
        'V0 must be finite and nonzero.');
end
if any(~isfinite([P_ref_pu Q_ref_pu E_ref_pu])) || E_ref_pu<=0
    error('ibr:decoupled_dual_mode_model:badRef', ...
...
gfm_dev=ibr.gfm_decoupled_full_model(device_id,bus_id,bus_position, ...
    bus_ids,V0,params,P_ref_pu,Q_ref_pu,E_ref_pu);

if gfl_dev.nx~=10 || gfm_dev.nx~=11
    error('ibr:decoupled_dual_mode_model:branchLayout', ...
        'Expected a 10-state GFL adapter and an 11-state decoupled GFM adapter.');
end

gx=gfl_dev.x0(:); mx=gfm_dev.x0(:);
if abs(gx(3)-mx(3))>1e-12
    error('ibr:decoupled_dual_mode_model:commonPlantInit', ...
...
gfl_active=1:9;
gfm_active=[1:3 10:17];

% Runtime hybrid_state field key, computed once (see the identical hoist note
% in eecon49_dual_mode_model: rebuilding it per closure call dominated the
% coupled FD Jacobian profile).
...
dev.device_type='ibr_decoupled_dual';
dev.mode=char(mode); dev.initial_mode=char(mode); dev.initial_online=true;
dev.nx=17; dev.nu=3; dev.state_names=state_names;
...
dev.provenance=struct( ...
    'model','DECOUPLED_GFM_GFL_SHARED_PLANT_DUAL', ...
    'source',['GFL branch and shared plant as mapped in ' ...
        'eecon49_dual_mode_model; GFM swing block PROJECT_DERIVED ' ...
        '(gfm_decoupled_full_model, washout damping)'], ...
    'classification',['PROJECT_DERIVED decoupled GFM swing over ' ...
        'SOURCE_MAPPED AC/control equations; PROJECT_DERIVED fixed ' ...
        'superset/transfer/DC regulator'], ...
    'details',['17 states = common plant 3 + GFL controller 6 (PLL) + ' ...
        'GFM controller 8 (decoupled VSG, no PLL, washout omega_f last); ' ...
        'active GFL=9 and active GFM=11; command-delay states reduced ' ...
        '(v_del=v_cmd, T_d<<dt); omega_f enters GFM tracked on transfer'], ...
    'omega_f_index',17);
end
...
case 'GFM'
    ...
    dx(1:3)=dm(1:3); dx(10:17)=dm(4:11);
end
if any(~isfinite(dx))
    error('ibr:decoupled_dual_mode_model:nonfiniteRhs', ...
        'Non-finite decoupled dual RHS.');
end
end
...
if ~isscalar(I) || ~isfinite(I)
    error('ibr:decoupled_dual_mode_model:nonfiniteCurrent', ...
        'Decoupled dual current injection must be one finite scalar.');
end
end
...
if ~online || strcmp(m,'tripped')
    if any(~isfinite([P Q])) || abs(P)>64*eps(max(1,abs(P))) || ...
        ...
        error('ibr:decoupled_dual_mode_model:offlineEquilibriumPower', ...
...
if strcmp(target,'GFM')
    ...
    % Washout state enters TRACKED so the transient-damping term
    % D_t*(omega - omega_f) is exactly zero at the transfer instant.  This
    % must follow the omega carry-over above, not precede it.
    mt(11)=mt(5);
...
if strcmp(target,'tripped')
    ...
    if abs(Iright-Ileft)>tol
        error('ibr:transfer_maps:currentContinuity', ...
            'Decoupled dual transfer current jump %.3g exceeds %.3g.', ...
...
info=struct('Vbus',V,'I_left',Ileft,'I_right',Iright, ...
    ...
    'common_indices',1:3,'gfl_indices',4:9,'gfm_indices',10:17, ...
...
% KEY is the constructor's status_key. Reading it straight from the argument
% removes the per-call matlab.lang.makeValidName that dominated the profile;
% the resolved field name is bit-for-bit the same string.
...
if ~isempty(ec) && isstruct(ec) && isfield(ec,'hybrid_state') && ...
    ...
    if isfield(hs,'device_online') && isstruct(hs.device_online) && ...
        ...
        if ~(islogical(v) && isscalar(v))
            error('ibr:decoupled_dual_mode_model:badRuntimeOnline', ...
...
if ~(ischar(raw) || (isstring(raw) && isscalar(raw)))
    error('ibr:decoupled_dual_mode_model:badRuntimeMode', ...
...
if ~isvector(u) || numel(u)~=3 || any(~isfinite(u))
    error('ibr:decoupled_dual_mode_model:badInput', ...
...
if ~isvector(x) || numel(x)~=17 || any(~isfinite(x))
    error('ibr:decoupled_dual_mode_model:badState', ...
        'Expected 17 finite decoupled-dual superset states.');
end
end
...
x(4:9)=g(4:9); x(10:17)=m(4:11);
end
function g=to_gfl(x), g=[x(1:9);0]; end
function m=to_gfm(x), m=[x(1:3);x(10:17)]; end
function x=put_gfl(x,g), x(1:3)=g(1:3); x(4:9)=g(4:9); end
function x=put_gfm(x,m), x(1:3)=m(1:3); x(10:17)=m(4:11); end
