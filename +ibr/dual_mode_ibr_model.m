...
% Forward the configured RMS10 runtime V_div_min as a scalar provenance field
% so the TS domain-preserving diagnostic can attribute a rejected trial to
% the exact device/bus without parsing exception text or hard-coding 0.1.
% Only the RMS10 GFL branch defines a runtime low-voltage division floor;
% the WECC branch has no such threshold and the field stays NaN.
gfl_runtime_min_voltage = NaN;
if isfield(gfl_dev,'provenance') && isstruct(gfl_dev.provenance) && ...
        isfield(gfl_dev.provenance,'params') && isstruct(gfl_dev.provenance.params) && ...
        isfield(gfl_dev.provenance.params,'V_div_min') && ...
        isscalar(gfl_dev.provenance.params.V_div_min) && ...
        isfinite(gfl_dev.provenance.params.V_div_min)
    gfl_runtime_min_voltage = gfl_dev.provenance.params.V_div_min;
end
dev.provenance.gfl_runtime_min_voltage = gfl_runtime_min_voltage;
end
