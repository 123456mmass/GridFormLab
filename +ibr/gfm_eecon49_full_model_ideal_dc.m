...
% Identical to gfm_eecon49_full_model except the DC-source closure, which reverts to the
...
function dev = gfm_eecon49_full_model_ideal_dc(device_id,bus_id,bus_position,bus_ids,V0,params,P_ref,Q_ref,E_ref)
...
% IDEAL-DC DIAGNOSTIC CLOSURE (ASSUMED_DIAGNOSTIC, comparison-only clone).
% Reverts the retired EECON49 ideal closure for this diagnostic clone only:
%   dVdc/dt = (Vdc0 - vdc)/Tdc, retired declared Tdc = 0.10 s
% which is structurally bounded -- Vdc can never reach the Pac/vdc
% singularity. That boundedness is the property the EECON49 paper's
% Fixed-GFL figure relied on. The I_dc state keeps the ABI by riding a
% first-order lag toward the bounded target (Vdc0-vdc)/Rdc; under this
% closure it no longer couples back into Vdc.
dc_rows=[(Vdc0-vdc)/0.10; ((Vdc0-vdc)/dcp.Rdc-idc)/dcp.tau_s];
