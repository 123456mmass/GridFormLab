function out = smib_loaded_sssa_oracle(dev,x_eq,V_eq,u_eq,V_inf,Z_line,P_load,Q_load,opt)
%SMIB_LOADED_SSSA_ORACLE  Independent loaded-IBR/infinite-bus SSSA characterization.
%   OUT = ibr.smib_loaded_sssa_oracle(DEV,X_EQ,V_EQ,U_EQ,V_INF,Z_LINE, ...
%   P_LOAD, Q_LOAD, OPT) treats the infinite bus as an algebraic voltage
%   source (no SG states) and adds a constant-power shunt load (P_LOAD,Q_LOAD)
%   at the terminal bus:
%
%       g(x,V) = I_ibr(x,V) - (V - V_inf)/Z_line
%                              - conj((P_load + j Q_load)/V) = 0.
%
%   The device uses its production f/current_injection closures unchanged.
%   This helper independently forms the centered-FD DAE Jacobian and Schur
%   complement
%
%       A = f_x - f_y*(g_y\g_x),
%
...
%
%   Mirrors ibr.smib_sssa_oracle exactly except for the load current term in
%   local_g. The existing ideal-SMIB oracle is NOT edited.
...
g_handle = @(x,y) local_g(dev,x,y,u_eq,ec,V_inf,Z_line,P_load,Q_load);
f0 = f_handle(x_eq,y_eq);
g0 = g_handle(x_eq,y_eq);
if numel(f0) ~= nx || numel(g0) ~= ny || any(~isfinite(f0)) || any(~isfinite(g0))
    error('ibr:smib_loaded_sssa_oracle:residualContract', ...
...
if ~isfinite(gy_rcond) || gy_rcond <= 1e-10
    error('ibr:smib_loaded_sssa_oracle:illConditionedGy', ...
        'Loaded-IBR algebraic gy rcond %.3e must exceed 1e-10.',gy_rcond);
end
A_full = fx - fy*(gy\gx);
...
out = struct();
out.classification = 'ASSUMED_DIAGNOSTIC_SMIB_LOADED_ORACLE';
...
if isempty(active) || any(~isfinite(active)) || any(active ~= fix(active)) || ...
    ...
    error('ibr:smib_loaded_sssa_oracle:activeStateIndices', ...
...
I_load = conj((complex(P_load,Q_load))/V);
...
for it = 1:opt.algebraic_max_iterations
    ...
    if rcond(J) <= 1e-12
        error('ibr:smib_loaded_sssa_oracle:algebraicJacobian', ...
            'Perturbed algebraic solve has ill-conditioned Jacobian.');
    end
    y = y-J\r;
    if any(~isfinite(y))
        error('ibr:smib_loaded_sssa_oracle:algebraicNonfinite', ...
            'Perturbed algebraic solve generated a non-finite voltage.');
    end
end
error('ibr:smib_loaded_sssa_oracle:algebraicNoConverge', ...
