...
if has_case
    ...
    % A post-trip selector must use the case post-trip dispatch unless the
    % caller explicitly supplies another nonempty dispatch.  A scalar empty
    % struct is not isempty() in MATLAB; treating it as a real dispatch used
    % to zero every non-reference IBR and overload the reference GFM.
    post_trip_context = isfield(eval_opt,'sg_online') && ...
        isscalar(eval_opt.sg_online) && ~logical(eval_opt.sg_online);
    if ~post_trip_context && isfield(scenario,'config') && ...
            isfield(scenario.config,'dispatch') && ...
            has_dispatch_values(scenario.config.dispatch) && ...
            ~has_dispatch_values_from_opt(eval_opt)
    ...
    if isfield(scenario,'scenario_opt') && isstruct(scenario.scenario_opt)
        ...
        if ~post_trip_context && isfield(scenario.scenario_opt,'dispatch') && ...
                has_dispatch_values(scenario.scenario_opt.dispatch) && ...
                ~has_dispatch_values_from_opt(eval_opt)
...
c = struct('resource_ids', {{}}, 'resource_type', {{}}, 'modes', {{}}, ...
    ...
    'fd_zeta_worsts', [], 'zeta_min', NaN, 'zeta_worst', NaN, ...
    'zeta_margin', NaN, ...
...
function tf = has_dispatch_values_from_opt(opt)
tf = isfield(opt,'dispatch') && has_dispatch_values(opt.dispatch);
end
