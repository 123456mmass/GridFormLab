...
- Researcher-set free parameters (owner authority). This is the project owner's own research: when a parameter, closure, or modeling constant is genuinely NOT fixed by the source — correctly classified `PROJECT_DERIVED` or `NUMERICAL_METHOD` (e.g. an actuation/command delay, controller time constant, or filter the source never published) — the owner may set it, choosing the best value on independent physical or mathematical grounds. Derive it from cited equations, device physics, a stability/delay-margin argument, or an in-repo derivation, and record that derivation — the proof, not merely the number — beside its classification. An equation-backed proof available on demand is the baseline professional standard for every value the project sets. State the choice's consequences (stability, margins, agreement) honestly, as verification of the choice and never as its hidden motivation. This authority covers only genuinely-free quantities and never overrides the rules below: it does not license altering a `SOURCE_DEFINED` or `CASE_DEFINED` value, changing anything merely to pass a test or fabricate agreement, or reverse-engineering a parameter from a desired result without an independent sourced-or-derived justification.
