function probe_sauer_pb3()
% Use current-balance equilibrium, then compute eigenvalues with power-balance g.
...
% Solve equilibrium using current-balance form (known to converge)
fixed_x=3; free_x=setdiff(1:numel(x0),fixed_x);
fixed_y=2; free_y=setdiff(1:numel(y0),fixed_y);
z0=[x0(free_x); y0(free_y); Tm; Vref];
resfun=@(z)residual_cb(z,x0,y0,free_x,free_y,mp,Y,Tm,Vref);
...
fprintf('PB residual: %.3e\n', norm([dae_f(x,y,mp,Tm,Vref); dae_g_pb(x,y,mp,Y)]));

% Compute eigenvalues using power-balance g
...
lam=r.eigenvalues;
fprintf('PB eigen: maxreal=%.4f\n', max(real(lam)));

% Also compute using current-balance g for comparison
model2=struct(); model2.x0=x; model2.y0=y;
model2.f=@(xx,yy)dae_f(xx,yy,mp,Tm,Vref);
model2.g=@(xx,yy)dae_g_cb(xx,yy,mp,Y);
model2.free_y=free_y; model2.fd_eps=1e-6;
r2=stability.multimachine_ssa(model2);
lam2=r2.eigenvalues;
fprintf('CB eigen: maxreal=%.4f\n', max(real(lam2)));
...
for src=1:2
  if src==1; ll=lam; nm='PB'; else; ll=lam2; nm='CB'; end
  ll_osc=ll(abs(imag(ll))>0.1 & imag(ll)>0);
  [~,idx]=sort(abs(imag(ll_osc)),'descend'); ll_osc=ll_osc(idx);
  fprintf('\n=== %s ===\n', nm);
  for k=1:min(numel(ll_osc),numel(ref_osc))
    fprintf('  %+8.4f%+8.4fj  ref %+8.4f%+8.4fj  err %.4f %.4f\n', ...
      real(ll_osc(k)), imag(ll_osc(k)), real(ref_osc(k)), imag(ref_osc(k)), ...
      abs(real(ll_osc(k))-real(ref_osc(k))), abs(imag(ll_osc(k))-imag(ref_osc(k))));
  end
end
end

function res=residual_cb(z,x0,y0,free_x,free_y,mp,Y,Tm0,Vref0)
...
res=[dae_f(x,y,mp,Tm,Vref); dae_g_cb(x,y,mp,Y)];
end
