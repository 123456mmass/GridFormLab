function probe_sauer_pb_final()
% Power-balance form with correct Ybus.
...
resfun=@(z)residual(z,x0,y0,free_x,free_y,mp,Y);
...
fprintf('res=%.3e exit=%d iter=%d\n', norm(resfun(zsol)), exitflag, out.iterations);
...
ref_real=ref(abs(imag(ref))<1e-6);
[~,idx]=sort(real(ref_real),'descend'); ref_real=ref_real(idx);
fprintf('Real modes:\n');
for k=1:min(numel(lam_real),numel(ref_real))
  fprintf('  %+8.4f  ref %+8.4f  err %.4f\n', real(lam_real(k)), real(ref_real(k)), abs(real(lam_real(k))-real(ref_real(k))));
end
end

function res=residual(z,x0,y0,free_x,free_y,mp,Y)
...
for k=1:ng
  ...
  f(ii+5)=(-mp(k).KE*Efd-0.0039*exp(1.555*Efd)*Efd+VR)/mp(k).TE;
...
for k=1:3
  mp(k)=struct('H',H(k),'M',2*H(k)/ws,'D',D(k),'Xd',Xd(k),'Xdp',Xdp(k),'Xq',Xq(k),'Xqp',Xqp(k), ...
    'Tdo',Tdo(k),'Tqo',Tqo(k),'KA',20,'TA',0.2,'KE',1.0,'TE',0.314,'KF',0.063,'TF',0.35);
end
end
