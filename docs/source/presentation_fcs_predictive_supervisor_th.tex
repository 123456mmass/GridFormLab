...
\setbeamertemplate{footline}{%
  ...
  \color{navy}\scriptsize แนวคิดสำหรับพัฒนาต่อ -- ยังไม่ใช่ผลการทดลอง
...
\begin{frame}{16. แนวคิดที่เราจะพัฒนาต่อ}
\begin{block}{แนวคิดหลัก}
ให้ AGSI++ ทำหน้าที่ตรวจจับความผิดปกติราย IBR แล้วใช้ ET-FCS predictive supervisor วิเคราะห์สภาวะทั้งโครงข่าย เพื่อเลือกชุด GFL/GFM และ reference leader ก่อนสั่งสลับจริง
\end{block}
\begin{columns}[T,onlytextwidth]
\column{.48\textwidth}
\textbf{สิ่งที่คาดว่าจะได้}
\begin{itemize}
  \item ตัดสินใจจากสภาวะทั้งระบบ ไม่ใช่ threshold อย่างเดียว
  \item เลือกจำนวนและตำแหน่ง GFM เท่าที่จำเป็น
  \item ลด transient และการสลับที่ไม่จำเป็น
  \item อธิบายเหตุผลของคำสั่งย้อนหลังได้
\end{itemize}
\column{.48\textwidth}
\textbf{ประเด็นที่ต้องพิสูจน์จากผลจำลอง}
\begin{itemize}
  \item prediction ต้องเร็วพอก่อนถึงเวลาสั่งสลับ
  \item voltage, frequency และ current transient ลดลงจริง
  \item ผลไม่ไวเกินไปต่อ horizon, weight และ $\Delta t$
  \item ทุก transaction ผ่าน KCL และไม่มี switching chatter
\end{itemize}
\end{columns}
\end{frame}
