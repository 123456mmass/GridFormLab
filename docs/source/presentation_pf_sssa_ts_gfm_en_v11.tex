...
\end{block}
\begin{itemize}
  \item $\mathrm{SG}_1$: synchronous reference
  \item $\mathrm{IBR}_1$--$\mathrm{IBR}_4$: dual-mode converters
\end{itemize}
\column{0.75\textwidth}
\vspace{0.17cm}
\centering
\makebox[\linewidth][c]{\includegraphics[width=1.00\linewidth]{figures/123313.png}}
\end{columns}
\end{frame}
...
\scriptsize
Net complex power injected at bus $i$
\[
P_i+jQ_i=V_i\left(\sum_{j=1}^{N}Y_{ij}V_j\right)^{\!*}
\]
which, with $V_i=|V_i|e^{j\theta_i}$, is the polar form used in Hadi Saadat
\begin{align*}
P_i&=\sum_{k=1}^{N}|V_i||V_k||Y_{ik}|\cos(\angle Y_{ik}+\theta_k-\theta_i),\\
Q_i&=-\sum_{k=1}^{N}|V_i||V_k||Y_{ik}|\sin(\angle Y_{ik}+\theta_k-\theta_i).
\end{align*}
...
=\begin{bmatrix}\bm J_{P\theta}&\bm J_{PV}\\\bm J_{Q\theta}&\bm J_{QV}\end{bmatrix}
\begin{bmatrix}\Delta\bm\theta\\\Delta|\bm V|\end{bmatrix}}
\]
\[
\bm\theta^{(r+1)}=\bm\theta^{(r)}+\Delta\bm\theta^{(r)},\qquad
...
\small
\textbf{Linearized DAE}
\[
\begin{bmatrix}\Delta\dot{\bm x}\\\bm 0\end{bmatrix}
=\begin{bmatrix}\bm f_x&\bm f_y\\\bm g_x&\bm g_y\end{bmatrix}
\begin{bmatrix}\Delta\bm x\\\Delta\bm y\end{bmatrix}
\]
With $\bm g_y$ nonsingular the algebraic perturbation is eliminated by the Schur complement, and no AVR/PSS perturbation input is applied:
\[
\boxed{\bm A_{red}=\bm f_x-\bm f_y\bm g_y^{-1}\bm g_x},\quad
\Delta\dot{\bm x}=\bm A_{red}\Delta\bm x
\]
\column{0.40\textwidth}
\small
\textbf{Modal analysis}
\[
\boxed{\bm A_{red}\bm\phi_i=\lambda_i\bm\phi_i}
\]
\[
\lambda=\sigma\pm j\omega_d,\qquad
f_m=\frac{|\omega_d|}{2\pi}
\]
\[
\zeta=-\frac{\sigma}{\sqrt{\sigma^2+\omega_d^2}}
\]
\begin{block}{Stability criterion}
$\operatorname{Re}(\lambda)<0$ for every physical decision root, together with the declared damping-ratio floor.
\end{block}
\end{columns}
\end{frame}
...
\IfFileExists{figures/switch_ieee14_decision/run_summary_v2.tex}{\input{figures/switch_ieee14_decision/run_summary_v2.tex}}{}
\IfFileExists{figures/switch_ieee14_decision/comparison_macros.tex}{\input{figures/switch_ieee14_decision/comparison_macros.tex}}{}
\section{Results}
...
2 & $\mathrm{IBR}_1+\mathrm{IBR}_3$ & admissible \\
2 & $\mathrm{IBR}_2+\mathrm{IBR}_3$ & \textbf{best margin} \\
...
\begin{frame}{Result 3: Electrical Response and Supervisor Decision}
\centering
\includegraphics[width=0.97\textwidth,height=0.72\textheight,keepaspectratio]{figures/presentation_pf_sssa_ts_gfm/slide_electrical_v5.png}
\par\vspace{0.40cm}
\scriptsize
(a) active power, (b) reactive power, (c) severity index $S$ against $\Gamma_{on}=0.65$ and $\Gamma_{off}=0.35$, (d) island angle-reference owner.
\end{frame}
\begin{frame}{Result 4: Bus-9 Response under Different Policies}
\centering
\includegraphics[width=0.97\textwidth,height=0.72\textheight,keepaspectratio]{figures/presentation_pf_sssa_ts_gfm/slide_bus9_v1.png}
\par\vspace{0.40cm}
\scriptsize
(a) $P_{\mathrm{Bus9}}$, (b) $Q_{\mathrm{Bus9}}$, (c) $V_{\mathrm{Bus9}}$, (d) converter-measured frequency $f_{\mathrm{conv}}$.
\end{frame}
...
\begin{itemize}
  ...
  \item After $\mathrm{SG}_1$ is disconnected, at least one voltage-forming IBR is required for the islanded trajectory to continue.
