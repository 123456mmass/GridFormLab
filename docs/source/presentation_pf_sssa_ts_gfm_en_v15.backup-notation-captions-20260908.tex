...
\centering
\begin{tikzpicture}[font=\footnotesize,b/.style={draw=barblue,rounded corners,align=center,text width=2.45cm,minimum height=1.0cm},a/.style={-{Stealth},thick,barblue},lbl/.style={font=\scriptsize,text=myblue,align=center,inner sep=2pt}]
\node[b] (v) at (0,0) {Measure\\$|V_i|,f_{coi}$};
\node[b] (s) at (3.4,0) {Index\\$S_i$};
\node[b] (h) at (6.8,0) {Threshold + dwell};
\node[b] (r) at (6.8,-2.5) {Readiness};
\node[b] (m) at (3.4,-2.5) {GFL / GFM};
\draw[a](v.east)--(s.west);
\draw[a](s.east)--(h.west);
\draw[a](h.south)--(r.north) node[lbl,midway,right=2mm]{request\\switch};
\draw[a](r.west)--(m.east);
\draw[a](m.west)-|(v.south);
\node[lbl,anchor=south] at (1.7,.75) {compute index};
\node[lbl,anchor=south] at (5.1,.75) {compare $\Gamma_{on}/\Gamma_{off}$};
\node[lbl,anchor=south] at (5.1,-1.75) {authorize mode change};
\node[lbl,anchor=north] at (1.7,-3.25) {feedback measurements};
\end{tikzpicture}
\par\bigskip
