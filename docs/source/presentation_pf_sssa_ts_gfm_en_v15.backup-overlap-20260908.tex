...
\begin{frame}{Switchable GFL/GFM Control Model}
	...
	\begin{columns}[t,onlytextwidth]
		...
		\vspace{.4cm}
		\begin{figure}
			\includegraphics[width=\linewidth,height=6.4cm,keepaspectratio]{figures/fig2_control_structure.png}
...
\centering
\begin{tikzpicture}[font=\footnotesize,b/.style={draw=barblue,rounded corners,align=center,text width=2.55cm,minimum height=1.0cm},a/.style={-{Stealth},thick,barblue},lbl/.style={font=\scriptsize,text=myblue,align=center,fill=white,inner sep=1.5pt}]
...
\draw[a](v)--(s) node[lbl,midway,above=1.5mm]{compute index};
\draw[a](s)--(h) node[lbl,midway,above=1.5mm]{compare $\Gamma_{on}/\Gamma_{off}$};
\draw[a](h)--(r) node[lbl,midway,right=1.5mm]{request switch};
\draw[a](r)--(m) node[lbl,midway,above=1.5mm]{authorize mode change};
\draw[a](m.west)-|(v.south) node[lbl,pos=.25,below=1.5mm]{feedback measurements};
\end{tikzpicture}
\par\bigskip
