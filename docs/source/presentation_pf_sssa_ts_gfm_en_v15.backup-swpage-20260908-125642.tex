...
\end{tabular}
\par\medskip
\textbf{PF and TS variables}\par\smallskip
\begin{tabular}{@{}p{1.65cm}>{\raggedright\arraybackslash}p{3.25cm}@{}}
...
\begin{columns}[t,onlytextwidth]
\column{.48\textwidth}
\textbf{Power flow: equilibrium}\par
...
\begin{equation}\dot x_{IBR}=\begin{cases}F_0(x_{IBR},y,u)&m_i=0,\\F_1(x_{IBR},y,u)&m_i=1.\end{cases}\end{equation}
...
\begin{equation}
\begin{aligned}
M\dot\omega_{VSG}&=\kappa P_{ref}-P_{inv}\\
&\quad-D_v(\omega_{VSG}-1)
\end{aligned}
\end{equation}
\scriptsize $M,D_v$: VSG inertia and damping.\par
\begin{equation}\kappa=S_{base}/M_{base}\end{equation}
\column{.45\textwidth}
\begin{figure}\includegraphics[width=\linewidth,height=3.5cm,keepaspectratio]{figures/fig2_control_structure.png}\caption{Switchable controller branches.}\label{fig:control15}\end{figure}
\end{columns}
\begin{equation}
x_{IBR}=[i_d,i_q,V_{dc},x_{GFL}^{T},x_{GFM}^{T},I_{dc}]^T
\end{equation}
\scriptsize 17 stored states; the inactive controller branch is frozen.\par
\end{frame}
...
\end{align}
\scriptsize $V_i^\star$: healthy PF voltage; $S_i$: severity index in $[0,1]$.\par
...
\end{equation}
\scriptsize $\mathcal L,\mathcal G$: online GFL/GFM sets. \par
\medskip
...
Index bases&$\Delta V_{base},\Delta f_{base}$&.10, .50&pu, Hz\\
\bottomrule
\end{tabular}\par
\medskip
\raggedright\scriptsize
...
\node[tag,anchor=west] at (7.62,0) {$t=20^{+}$ s};
