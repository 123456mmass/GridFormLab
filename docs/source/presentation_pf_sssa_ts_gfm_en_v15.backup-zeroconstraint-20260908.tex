...
\textbf{Shared electrical, power-flow and time-domain symbols}\par\smallskip
\begin{tabular}{@{}p{3.1cm}>{\raggedright\arraybackslash}p{7.4cm}@{}}
$V_i,\theta_i$ & bus voltage magnitude / angle\\
$f_{coi},f_0,\omega_b$ & centre-of-inertia / nominal frequency; $\omega_b=2\pi f_0$\\
$Y$ & network admittance\\
	$i_d,i_q,V_{dc},I_{dc}$ & converter plant (vectors on IBR page)\\
	$\delta,\omega,SG EMFs$ & SG rotor / internal EMFs\\
$P_j,Q_j$ & net bus injection, as in the power-flow equation\\
$x,x_{SG},x_{IBR},y,u$ & machine+converter states; SG / IBR vectors; bus voltages; references, loads, events\\
$m_i$ & IBR $i$ mode: 0 = GFL, 1 = GFM\\
$F_0,F_1$ & 17-state branch maps; inactive branch held\\
$g=0,t$ & current balance at every bus, in PF and TS; time\\
\end{tabular}\par\medskip
\textbf{Controller symbols}\par\smallskip
\begin{tabular}{@{}p{3.1cm}>{\raggedright\arraybackslash}p{7.4cm}@{}}
...
$\theta_{VSG},\omega_{VSG},E$ & VSG angle / speed / voltage magnitude\\
$\xi_{Vd},\xi_{Vq}$ & voltage-loop integrators\\
$k_p,k_i$ & PI gains; $M,D_v$: VSG inertia / damping\\
\end{tabular}
\end{frame}
...
{\scriptsize
\begin{align}
P_j&=\sum_k|V_j||V_k||Y_{jk}|\cos(\theta_j-\theta_k-\phi_{jk}),\\
Q_j&=\sum_k|V_j||V_k||Y_{jk}|\sin(\theta_j-\theta_k-\phi_{jk}).
\end{align}
\vspace{-.2cm}
{\tiny $\phi_{jk}=\angle Y_{jk}$.\par}}
\column{.48\textwidth}
...
\centering\begin{figure}\includegraphics[width=\linewidth,height=6.1cm,keepaspectratio]{figures/ieee14_scenario_suite/sg_fault_cycle160_ibr_states_v15.png}\caption{IBR$_1$ states: $(a)$ $i_d,i_q$; $(b)$ $V_{dc},I_{dc}$; $(c)$ $\omega_{VSG}$; $(d)$ $E$}\end{figure}
{\scriptsize Blue = active; grey dashed = held stored state. IBR$_1$ only.\par}
\end{frame}
\begin{frame}{Result: Index and Mode}
\centering
\begin{figure}\includegraphics[width=.76\linewidth]{figures/ieee14_scenario_suite/sg_fault_cycle160_agsi3d.png}\caption{Severity index $S_i$ per converter.}\end{figure}
\vspace{-.2cm}
\begin{figure}\includegraphics[width=.76\linewidth]{figures/ieee14_scenario_suite/sg_fault_cycle160_mode3d.png}\caption{Converter mode $m_i$ per converter: GFL = 0, GFM = 1.}\end{figure}
\end{frame}
\begin{frame}{Result: Voltage and Frequency}
\centering\begin{figure}\includegraphics[width=\linewidth,height=6.45cm,keepaspectratio]{figures/ieee14_scenario_suite/sg_fault_cycle160_vf.png}\caption{Converter-bus voltage and per-converter frequency}\end{figure}
\end{frame}
\begin{frame}{Result: Active and Reactive Power}
\centering\begin{figure}\includegraphics[width=\linewidth,height=6.45cm,keepaspectratio]{figures/ieee14_scenario_suite/sg_fault_cycle160_pq_mode.png}\caption{Active and reactive power per converter}\end{figure}
\end{frame}
\begin{frame}{Fault Bus  Voltage and Frequency}
\centering\begin{figure}\includegraphics[width=\linewidth,height=6.2cm,keepaspectratio]{figures/ieee14_scenario_suite/sg_fault_cycle160_cmp_vf.png}\caption{Bus-9 voltage and frequency: adaptive vs locked-GFL diagnostic.}\end{figure}
\end{frame}
\begin{frame}{Fault Bus Active and Reactive Power}
\centering\begin{figure}\includegraphics[width=\linewidth,height=6.2cm,keepaspectratio]{figures/ieee14_scenario_suite/sg_fault_cycle160_cmp_pq.png}\caption{Bus-9 active and reactive power: adaptive vs locked-GFL diagnostic.}\end{figure}
\end{frame}
