...
%
% FIGURE NUMBERING: the Introduction TikZ concept diagram is Figure 1; the
% electrical model is Figure 2 and the control structure Figure 3. NOTE: the
% red dashed box inside fig1_ibr_electrical.png still points at "รูปที่ 2",
% which no longer matches -- the control structure is now Figure 3.
%
...
\item Increasing  IBR penetration.
\item Less synchronous-machine inertia.
\item Weak grids challenge PLL-based  GFL control.
\item GFM control provides voltage and frequency support.
...
\end{tikzpicture}
\caption{GFL/GFM mode-switching concept.}
\end{figure}
\end{columns}
\end{frame}
...
\begin{align}
	...
	x_{IBR}&=[x_{p}^T,x_{GFL}^T,x_{GFM}^T]
\end{align}
\end{frame}
...
\column{.43\textwidth}
\begin{figure}\includegraphics[width=\linewidth,trim=0 0 0 2mm,clip]{figures/xxxx.png}\caption{IBR electrical model.}\label{fig:plant}\end{figure}
...
\centering
\begin{figure}
\begin{tikzpicture}[font=\footnotesize,b/.style={draw=barblue,rounded corners,align=center,text width=2.35cm,minimum height=1.15cm},a/.style={-{Stealth},thick,barblue},lbl/.style={font=\tiny,text=myblue,align=center,inner sep=2pt}]
\node[b] (v) at (-3.15,0) {Measure $|V_i|,f_{coi}$\\at all IBR buses};
\node[b] (j) at (0.2,0) {Compute indices\\$J_{V,i},J_f$ (20), (21)};
\node[b] (s) at (3.55,0) {Severity index\\$S_i$ (22)};
\node[b] (h) at (3.55,-2.55) {Compare $\Gamma_{on}/\Gamma_{off}$ + dwell $T_{d,on}/T_{d,off}$ (23), (24)};
\node[b] (q) at (0.2,-2.55) {GFM / GFL\\request};
\node[b] (r) at (-3.15,-2.55) {Readiness check\\(references, feasible transfer)};
\node[b] (m) at (-3.15,-4.9) {Authorize mode change\\update $m_i$: GFL = 0, GFM = 1 (6)};
...
\draw[a](m.west)--++(-0.4,0)|-(v.west) node[lbl,pos=0.22,left]{feedback};
\node[lbl,text width=3.4cm] at (1.35,-4.9) {A switch is requested only when severity stays past the threshold for the full dwell time, then it is authorized only if the target device passes the readiness check; otherwise the mode is kept.};
\end{tikzpicture}
...
\centering
\begin{table}
\caption{Network and plant parameters.}
...
* Table References: [2], [3] \par

\end{frame}
\begin{frame}{Parameter Settings: Control and Supervisor}
\scriptsize
\renewcommand{\arraystretch}{1.06}\setlength{\tabcolsep}{3pt}
\centering
\begin{table}
\caption{Controller and supervisor parameters.}
\begin{tabular}{@{}p{3.05cm}p{3.0cm}p{2.55cm}p{1.25cm}@{}}
\toprule Parameter & Symbol & Value & Unit\\\midrule
...
\end{tikzpicture}
\caption{Scenario timeline.}
\end{figure}
\end{frame}
\begin{frame}{Result: Severity Index and Converter Mode}
\centering
\begin{figure}\includegraphics[width=.68\linewidth]{figures/ieee14_scenario_suite/sg_fault_cycle160_agsi3d.png}\caption{Severity index $S_i$ per converter.}\end{figure}

\vspace{1mm}
\begin{figure}\includegraphics[width=.68\linewidth]{figures/ieee14_scenario_suite/sg_fault_cycle160_mode3d.png}\caption{Converter mode $m_i$ per converter: GFL = 0, GFM = 1.}\end{figure}
\end{frame}
\begin{frame}{Result: IBR State Response}
	\centering\begin{figure}\includegraphics[width=\linewidth,height=6.60cm,keepaspectratio]{figures/ieee14_scenario_suite/sg_fault_cycle160_ibr_states_v15.png}\caption{$(a)$ $i_d,i_q$; $(b)$ $V_{dc}$; $(c)$ $\omega_{PLL}$; $(d)$ $\omega_{VSG}$}\end{figure}
\end{frame}
\begin{frame}{Result: Voltage and Frequency}
\centering\begin{figure}\includegraphics[width=\linewidth,height=5.9cm,keepaspectratio]{figures/ieee14_scenario_suite/sg_fault_cycle160_vf.png}\caption{Converter responses: $(a)$ bus-voltage magnitude and $(b)$ converter frequency.}\end{figure}
\end{frame}
\begin{frame}{Result: Active and Reactive Power}
\centering\begin{figure}\includegraphics[width=\linewidth,height=5.9cm,keepaspectratio]{figures/ieee14_scenario_suite/sg_fault_cycle160_pq_mode.png}\caption{Converter power responses: $(a)$ active power and $(b)$ reactive power.}\end{figure}
\end{frame}
\begin{frame}{Fault Bus  Voltage and Frequency}
\centering\begin{figure}\includegraphics[width=\linewidth,height=5.75cm,keepaspectratio]{figures/ieee14_scenario_suite/sg_fault_cycle160_cmp_vf.png}\caption{Bus-9 responses for adaptive GFL/GFM and fixed-GFL cases: $(a)$ voltage magnitude and $(b)$ frequency.}\end{figure}
\end{frame}
\begin{frame}{Fault Bus Active and Reactive Power}
\centering\begin{figure}\includegraphics[width=\linewidth,height=5.75cm,keepaspectratio]{figures/ieee14_scenario_suite/sg_fault_cycle160_cmp_pq.png}\caption{Bus-9 power responses for adaptive GFL/GFM and fixed-GFL cases: $(a)$ active power and $(b)$ reactive power.}\end{figure}
\end{frame}
