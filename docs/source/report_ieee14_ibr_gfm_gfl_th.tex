...
\newcommand{\pending}{\textcolor{red!75!black}{\textbf{PENDING PHASE 0B--5}}}
...
\centering
\textbf{สถานะเอกสาร: SOURCE-CLOSED DRAFT}\par
...
ส่วน nonlinear PLL-resolved GFL ใหม่และผลเชิงตัวเลข PF/SSSA/TS
ยังเป็น \pending{} และจะไม่ถูกแต่งขึ้นเพื่อให้รายงานดูสมบูรณ์
\end{minipage}}
\end{center}
...
ดังนั้นรายงานจะไม่กล่าวอ้าง GFL PLL participation จนกว่า Phase 0B จะพบ source
nonlinear positive-sequence GFL ที่ครบถ้วนและ Phase 4--5 จะ implement และตรวจสอบผ่าน
\end{abstract}
...
\begin{table}[H]\centering
\caption{สถานะส่วนประกอบ ณ source tree \code{7c72b94}}
...
PLL-resolved nonlinear GFL & \pending & Fu et al. รองรับ linear diagnostic เท่านั้น \\
...
\begin{align}
e_V&=V_{ref}-V_f, & \dot x_E&=e_V\quad\text{(conditional hold at outward saturation)},\\
...
\dot I_{q,reg}&=\frac{I_{q,cmd,f}-I_{q,reg}}{T_g},
\end{align}
...
\section{PLL-resolved Nonlinear GFL ใหม่}

\begin{center}
\fcolorbox{red!70!black}{red!3}{\parbox{.9\textwidth}{
\centering\pending\par
Fu et al. ให้ small-signal transfer functions แต่ไม่ให้ nonlinear PLL/PQ/current
ODEs, saturation, anti-windup, equilibrium initialization และ large-disturbance
validity ที่ครบถ้วน จึงยัง freeze state order ใหม่ไม่ได้}}
\end{center}

source ที่จะปลดบล็อกส่วนนี้ต้องให้ครบ: nonlinear SRF-PLL, gains/base/freeze/limits,
P/Q measurements และ outer loops, current dynamics, dq/network transform,
current priority, anti-windup, equilibrium, low-voltage behavior, parameter table
และ positive-sequence validity domain โมเดลใหม่ต้องเป็นไฟล์ใหม่และ opt-in โดยไม่เปลี่ยน
ความหมายของ WECC model เดิม

\begin{table}[H]\centering\small
\caption{Candidate state checklist --- ไม่ใช่ state order ที่อนุมัติแล้ว}
\begin{tabularx}{\textwidth}{p{.22\textwidth}Xl}
\toprule
Candidate state/block & สิ่งที่ source ต้องกำหนด & สถานะ \\\midrule
$\delta_{PLL},\xi_{PLL}$ & phase detector, ODE, gains, wrap/freeze/limits/init & \pending \\
$P_f,Q_f$ & measurement definition, base และ time constants & \pending \\
$\xi_P,\xi_Q$ & outer-loop error, gains, saturation/anti-windup & \pending \\
$I_d,I_q$ หรือ current-loop states & current-command dynamics และ network injection & \pending \\
Mode-transfer initialization & physical continuity และ exact state initialization & \pending \\
\bottomrule
\end{tabularx}
\end{table}
...
ไม่ใช่การ rerun ใหม่เพื่อสร้างรายงานฉบับนี้ และไม่เปลี่ยนสถานะ nonlinear GFL source gap
...
\item ต้องปิด Phase 0B source checklist ก่อน freeze state order ของ PLL-resolved GFL
\item Phase 4 ต้องสร้าง GFL ใหม่เป็น opt-in model โดยไม่ mutate WECC semantics
...
PF/equilibrium, full-state SSSA และ TS ถูกจัดวางแล้วในระดับเดียวกับรายงาน Padiyar
สิ่งที่ตั้งใจเว้นว่างไม่ใช่ข้อมูลตกหล่น แต่เป็น fail-closed boundary ต่อ source gap:
จนกว่าจะมี nonlinear PLL-resolved GFL source และ implementation ที่ผ่าน Phase 4--5
จะไม่มีการกล่าวอ้าง GFL PLL mode, state order หรือผล PF/SSSA/TS ของโมเดลใหม่
