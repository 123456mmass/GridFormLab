...
\newcommand{\implemented}{\textcolor{green!50!black}{\textbf{IMPLEMENTED (normal op)}}}
...
\centering
\textbf{สถานะเอกสาร: SOURCE-TRACEABLE DESIGN + IMPLEMENTED (GFL-RMS10 normal operation)}\par
...
พร้อมโมเดล nonlinear positive-sequence GFL--RMS10 ที่มี PLL และ current states จริงจาก
Yazdani--Iravani, Teodorescu--Liserre--Rodríguez และ Bacha--Munteanu--Bratcu
โมเดล GFL--RMS10 ได้ implement และผ่าน Phase 4 แล้วสำหรับ normal-operation slice
(PF/equilibrium/SSSA/TS ผ่าน shared kernels; Profile B = SG1 + GFM\_IBR2 + 3$\times$RMS10
converge และ event-free TS no-drift) ส่วน fault/LVRT TS ยังอยู่นอกขอบเขต
ผลเชิงตัวเลขจะไม่ถูกแต่งขึ้นเพื่อให้รายงานดูสมบูรณ์
\end{minipage}}
\end{center}
...
current injection และ equilibrium map ครบ และได้ implement เป็น
\texttt{+ibr/gfl\_rms10\_model.m} ที่ plug เข้า generic composite-device ABI เดียวกับ
SG EMF6 และ REGFM\_B1 โดยไม่มี GFL-specific solver ผล Profile B (SG1 + GFM\_IBR2 +
3$\times$GFL--RMS10) ผ่าน equilibrium/SSSA/TS ผ่าน shared kernels แล้ว
ส่วน fault/LVRT TS ยังอยู่นอกขอบเขตของ slice นี้
\end{abstract}
...
PLL-resolved GFL--RMS10 & \implemented & 10-state opt-in (\texttt{+ibr/gfl\_rms10\_model.m}); 6 SOURCE\_DEFINED + 4 PROJECT\_DERIVED states; Profile B ผ่าน PF/SSSA/TS ผ่าน shared kernels; fault/LVRT ยัง out-of-scope \\
...
Readiness & RMS10\_NORMAL\_OPERATION\_READY & fault/LVRT ยัง NOT\_READY; ห้ามกล่าวอ้าง LVRT จนกว่าจะมี source แยก \\
\bottomrule
\end{tabularx}
\end{table}
...
\paragraph{ความหมายของ clamp ในมุม VSM.}
operator $\clamp(z,\ell,h)$ บังคับค่า $z$ ให้อยู่ในช่วงปิด $[\ell,h]$:
\begin{equation}
\clamp(z,\ell,h)=\min\!\bigl(\max(z,\ell),h\bigr)
=\begin{cases}
\ell,&z<\ell,\\
z,&\ell\le z\le h,\\
h,&z>h.
\end{cases}
\label{eq:clamp-definition}
\end{equation}
ดังนั้นสมการมุมเขียนเต็มได้เป็น
\begin{equation}
\delta_{VSM}=\begin{cases}
\delta_{PLL}+\delta_{IT}^{min},&\delta_{IT}<\delta_{IT}^{min},\\
\delta_{PLL}+\delta_{IT},&\delta_{IT}^{min}\le\delta_{IT}\le\delta_{IT}^{max},\\
\delta_{PLL}+\delta_{IT}^{max},&\delta_{IT}>\delta_{IT}^{max}.
\end{cases}
\label{eq:vsm-angle-piecewise}
\end{equation}
$\delta_{PLL}$ คือมุม terminal/grid ที่ PLL ประมาณและใช้เป็น measurement frame;
$\delta_{IT}$ คือ virtual-inertial displacement เทียบกับ PLL; $\delta_{IT}^{min}$ และ
$\delta_{IT}^{max}$ คือ dynamic lower/upper bounds; ส่วน $\delta_{VSM}$ คือมุมสุดท้าย
ของ internal voltage phasor $E_{VSM}e^{j\delta_{VSM}}$ ที่ใช้คำนวณ current injection
clamp นี้ไม่ใช่การ wrap มุมเข้า $[-\pi,\pi]$ และไม่ได้จำกัด $\delta_{PLL}$ โดยตรง

ตัวอย่าง ถ้า $\delta_{PLL}=20^\circ$ และ bounds เท่ากับ $[-10^\circ,10^\circ]$:
$\delta_{IT}=5^\circ$ ให้ $\delta_{VSM}=25^\circ$;
$\delta_{IT}=15^\circ$ ถูก clamp เป็น $10^\circ$ จึงให้ $30^\circ$;
และ $\delta_{IT}=-18^\circ$ ถูก clamp เป็น $-10^\circ$ จึงให้ $10^\circ$
วัตถุประสงค์คือจำกัดความต่างมุมระหว่าง internal voltage กับ terminal voltage
เพื่อไม่ให้ current demand จาก voltage-behind-impedance relation สูงเกิน converter capability

ใน interior regime มี
$\dot\delta_{VSM}=\dot\delta_{PLL}+\dot\delta_{IT}=\omega_0\omega_m$
แต่เมื่อ clamp ทำงาน ถ้า bound หยุดนิ่งจะได้
$\dot\delta_{VSM}=\dot\delta_{PLL}$; ถ้า bound เคลื่อนที่จะได้
$\dot\delta_{VSM}=\dot\delta_{PLL}+\dot\delta_{IT}^{max}$ หรือ
$\dot\delta_{PLL}+\dot\delta_{IT}^{min}$ ตามขอบที่ active
จึงต้อง linearise ด้วย fixed active regime ไม่ใช้ smooth interior identity ครอบทุกกรณี
