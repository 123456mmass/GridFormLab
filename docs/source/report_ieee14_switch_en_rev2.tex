...
so the pair returns. At this dispatch $E_{dc}=1.0453\pu$ still lies below the
chopper threshold $V_{dc}^{\max}=1.10\pu$, but that is now a statement about the
...
columns, the modal frequency $f$, the damping ratio $\zeta$, and the three
largest normalised participation entries with their factors in parentheses.
...
$\zeta=1$. The participation factors are what make the last column readable as
evidence: three entries near $0.13$ each describe a shared family, whereas a
single entry at $1.000$ describes a coordinate that no other state drives.
Participation is a reporting diagnostic and enters no gate. It is the generated
table itself, not a re-typeset copy.
