...
%  CORRECTED 2026-08-26 (this revision):
%    - The DC-source closure was completed. The state table already listed I_dc
%      as index 17 and the active counts as 10/11, but the equations had been
%      left at the tau_s = 0 revision: row (3) still substituted the algebraic
%      law dVdc/dt = (1/C)[(Edc-Vdc)/Rdc - Pac/Vdc - Ich], and the word tau_s
%      did not appear anywhere in the file. No equation governed the state the
%      report's own table declared. Row (3) now reads I_dc, a new row (4) is the
%      Thevenin source law tau_s dIdc/dt = (Edc-Vdc)/Rdc - I_dc, and the
%      derivations of Rdc (from the declared regulation eps, bounded by the
%      reachability condition) and of tau_s (from the maximally-flat criterion,
%      bounded on both sides) were ported from the production derivation in
%      +ibr/dc_source_thevenin_params.m.
%    - The monotone bound Vdc <= max(Vdc(0),Edc) was removed. With I_dc a state
%      that lags its own static characteristic, row (3) no longer has a sign
%      that can be read off Vdc alone, and a zeta = 1/sqrt2 pair overshoots its
%      target by about 4.3%. What survives is the mechanism, not the inequality.
%    - The SSSA DC prose described four REAL roots just above -100 1/s with
%      participation 1.000 -- the resistive-source reading -- while the section
%      included the generated table that shows four complex conjugate pairs near
%      15 Hz with participation split 0.500/0.500. All of it was corrected
%      against measurement on the configuration that table prints (selected=[2],
%      reference=2, physical_A 40x40).
%    - EQUATION RENUMBERING: this revision ADDS four numbered equations (row (4)
%      of the shared power stage, plus eq:dcdisc, eq:dcblock, eq:dctau, and
%      eq:dclam replaces nothing). Cross-references use \eqref and update
%      themselves; only a reader citing a printed number is affected.
...
\IfFileExists{figures/switch_ieee14_decision/run_summary_v2.tex}
  ...
  {\newcommand{\NewRunEnd}{250.000}\newcommand{\NewRunVMin}{--}%
   ...
   % มาโครการเลือกชุดใหม่หลังต่อกลับ (เพิ่มพร้อมการส่งคืนที่ทำงานได้):
   % จำนวนครั้งและช่วงเวลาของ commit แรก ถ้าไฟล์ scalars ยังไม่มีให้ตกเป็น
   % เส้นประ ไม่ใช่ค่าเก่า
...
สองโหมดขนาด 17 สเตต พร้อมชุดสเตตที่แอ็กทีฟจริงในแต่ละโหมด และการถ่ายโอนโหมดที่มี
...
$I_{dc}$ & กระแสของแหล่งจ่าย DC เป็นสเตต ดัชนี 17 (ใช้ร่วมทั้งสองโหมด) \\
...
$L_s$, $\tau_s$ & ความเหนี่ยวนำของวงจรแหล่งจ่าย DC และค่าคงตัวเวลา $\tau_s=L_s/R_{dc}$ \\
$\varepsilon$ & ค่าการกำกับแรงดันของแหล่งจ่าย DC ที่กำลังพิกัด $P_r$ \\
...
\emph{ก่อน}ที่นิวตันจะถูกเรียกแม้ครั้งเดียว และผลของมันวัดได้: แขนที่ล็อก IBR ทุกตัว
ไว้ที่ GFL ถูกปฏิเสธที่ $t=20.000\second$ พอดี ซึ่งเป็นวินาทีที่ตัด SG
...
%=============================================================================
\section{อินเวอร์เตอร์: แบบจำลองสองโหมดขนาด 17 สเตต}\label{sec:ibr}
%=============================================================================
IBR แต่ละตัวเป็น\emph{อุปกรณ์ชิ้นเดียว}ที่สามารถแสดงตัวควบคุมได้สองแบบบนภาคกำลัง
ร่วมชุดเดียวกัน เวกเตอร์สเตตเต็มมีขนาดคงที่ 17 ตัว
\begin{equation}
  x_{\mathrm{IBR}}
   =\bigl[\underbrace{i_d,\,i_q,\,V_{dc}}_{(3)\;\text{แอ็กทีฟตลอด}}\;\big|\;
          \underbrace{x_{\mathrm{GFL}}\;(6)}_{\substack{\text{ดัชนี }4\text{--}9\\
            \text{แอ็กทีฟเฉพาะ GFL}}}\;\big|\;
          \underbrace{x_{\mathrm{GFM}}\;(7)}_{\substack{\text{ดัชนี }10\text{--}16\\
            \text{แอ็กทีฟเฉพาะ GFM}}}\;\big|\;
          \underbrace{I_{dc}}_{\substack{\text{ดัชนี }17\\
            \text{แอ็กทีฟตลอด}}}\bigr]^{\top}
   \in\mathbb{R}^{17},
   \label{eq:ibrstate}
\end{equation}
...
\end{align}
บล็อกภาคกำลังคือ $x_{\mathrm{pl}}=(i_d,i_q,V_{dc},I_{dc})$ และ\emph{ไม่ต่อเนื่อง}
กัน: กระแสแหล่ง DC อยู่ที่ดัชนี 17 หลังบล็อกตัวควบคุมสองชุด เพราะถูกเพิ่มเข้ากับ
เลย์เอาต์ที่ดัชนี 1--16 เผยแพร่ไปแล้ว การต่อท้ายแทนการแทรกทำให้ดัชนีเดิมทุกตัวยังหมาย
ความตรงเดิม ซึ่งเป็นเหตุผลที่ช่วง $4$--$9$ และ $10$--$16$ ในหัวข้อนี้ไม่เปลี่ยนจาก
รอบก่อน ๆ
...
\begin{equation}
  ...
  \label{eq:ibractive}
\end{equation}
กล่าวคือ 10 สเตตแอ็กทีฟในโหมด GFL, 11 สเตตในโหมด GFM และไม่มีสเตตใดแอ็กทีฟเมื่อ
อุปกรณ์ถูกตัดออก จุดตัด
$\mathcal{A}_{\mathrm{GFL}}\cap\mathcal{A}_{\mathrm{GFM}}=\{1,2,3,17\}$ คือภาค
กำลังร่วมพอดี และยูเนียนคือทั้งชุด $\{1{:}17\}$ บล็อกภาคกำลังร่วมแอ็กทีฟตลอดเวลา
ส่วนบล็อกตัวควบคุมสองชุดใช้งานแบบไม่ทับซ้อนกัน (mutually exclusive) จำนวนสเตต
แอ็กทีฟจึงเป็น $10$ หรือ $11$ ไม่เคยเป็น $17$ --- มิติซูเปอร์เซ็ตไม่เคยเป็นมิติ
พลวัต

ตารางที่~\ref{tab:ibrstates} แจกแจงทั้ง 17 สเตตตามลำดับดัชนีจริง พร้อมระบุว่าสเตต
...
\begin{table}[H]
\centering\footnotesize
\caption{แจกแจงสเตตทั้ง 17 ตัวของ IBR ตามลำดับดัชนีในโค้ด: ดัชนี 1--3 คือภาคกำลัง
ร่วมที่แอ็กทีฟทั้งสองโหมด ดัชนี 4--9 คือตัวควบคุม GFL ดัชนี 10--16 คือตัวควบคุม
GFM และดัชนี 17 คือกระแสแหล่ง DC ที่แอ็กทีฟทั้งสองโหมด}
\label{tab:ibrstates}
...
\midrule
\multicolumn{5}{@{}l}{\textit{แหล่ง DC (แอ็กทีฟทั้งสองโหมด)}}\\
17 & $I_{dc}$ & กระแสแหล่งจ่าย DC วงจร Thevenin & $\bullet$ & $\bullet$ \\
\midrule
\multicolumn{3}{@{}l}{\textit{จำนวนสเตตแอ็กทีฟรวม}} & \textit{10} & \textit{11} \\
\bottomrule
\end{tabular}
\end{table}
...
และแต่ละกิ่งเดี่ยวลดจาก $12$ เหลือ $10$ สเตต จากนั้นกระแสแหล่ง DC
$I_{dc}$ (ดัชนี 17 ของ~\eqref{eq:ibrstate} ดูการอนุมานวงจรที่ย่อหน้าแถว~(3)--(4)
ด้านล่างและหัวข้อการมีส่วนร่วมของ DC) เพิ่มอีกหนึ่งสเตต ให้ขนาด $17$ และ $11$
ต่อกิ่งเดี่ยว
...
\subsection{ภาคกำลังร่วม (สเตต 1--3 และ 17)}
พลวัตกระแสของตัวกรองแบบ $L$ บัสไฟตรง และกระแสของแหล่งจ่าย DC ใช้ร่วมกันทั้งสองโหมด
และแอ็กทีฟตลอดเวลา เขียนหนึ่งสเตตต่อหนึ่งบรรทัดตามลำดับดัชนี
\begin{align}
  ...
  C\,\dot{V}_{dc} &= I_{dc}-\frac{P_{ac}}{V_{dc}}
      -\frac{\max(0,\,V_{dc}-V_{dc}^{\max})}{R_{ch}},
    &&\text{(3) แรงดันบัส DC}\\
  \tau_s\,\dot{I}_{dc} &= \frac{E_{dc}-V_{dc}}{R_{dc}}-I_{dc},
    &&\text{(4) กระแสแหล่งจ่าย DC (ดัชนี 17)}
\end{align}
...
$\tau_s=L_s/R_{dc}$ คือค่าคงตัวเวลาของวงจรแหล่งจ่ายซึ่งเกิดจากความเหนี่ยวนำ $L_s$ ของมัน,
...
แถว (3)--(4) คือการปิดสมการแหล่งจ่าย DC แบบ \textsf{PROJECT\_DERIVED}: กรณีศึกษาให้เพียง
...
DC --- ในอันดับแรกคือแรงเคลื่อนไฟฟ้าหลังความต้านทานภายในรวมสายเคเบิล และวงจรนั้น
\emph{มีความเหนี่ยวนำ} ด้วย กระแสที่มันจ่ายได้จริงจึงไม่เท่ากับลักษณะสมบัติสถิตของ
ตัวเองในทันที แต่วิ่งไล่ตามด้วยค่าคงตัวเวลา $\tau_s=L_s/R_{dc}$ ตามแถว (4) นั่นคือ
เหตุผลที่ $I_{dc}$ เป็น\emph{สเตต} ไม่ใช่ปริมาณพีชคณิต ส่วนกระแสชอปเปอร์กันแรงดันเกิน
$I_{ch}=\max(0,V_{dc}-V_{dc}^{\max})/R_{ch}$ ถูกหักออกในแถว (3)

การตั้ง $\tau_s=0$ คืนกฎพีชคณิต $I_{dc}=(E_{dc}-V_{dc})/R_{dc}$ กลับมา\emph{พอดี}
ไม่ใช่โดยการประมาณ (ดูข้อความเรื่องความเสื่อมถอยด้านล่าง) ดังนั้นผู้อ่านที่เทียบหัวข้อนี้
กับรายงานฉบับก่อนหน้าซึ่งพิมพ์แถว DC เป็นสมการสเกลาร์แถวเดียวกำลังเทียบแบบจำลองหนึ่ง
กับลิมิตของมันเอง ไม่ใช่เทียบแบบจำลองสองตัว
...
\emph{จุดสมดุล} การให้ $\dot V_{dc}=\dot I_{dc}=0$ ขณะชอปเปอร์ปิดให้
...
ไปตาม $P_{ac}$ ที่ภาระอื่นทุกค่า สังเกตว่าเงื่อนไขนี้ไม่เปลี่ยนจากแบบจำลองพีชคณิต
เพราะที่จุดสมดุล แถว (4) บังคับ $I_{dc}=(E_{dc}-V_{dc})/R_{dc}$ อยู่แล้ว การเพิ่มสเตต
เปลี่ยน\emph{พลวัต} ไม่ได้เปลี่ยน\emph{จุดสมดุล}

\paragraph{$R_{dc}$ มาจากค่าการกำกับที่ประกาศไว้ และถูกจำกัดจากด้านบนด้วยเงื่อนไข
ความเข้าถึงได้}
ให้ $\varepsilon$ เป็นค่าการกำกับแรงดันของแหล่งจ่าย DC คือสัดส่วนที่แรงดัน DC ยกขึ้น
จากจุดที่สั่งไว้ไปถึงสภาวะไร้โหลด ที่กำลังพิกัดของคอนเวอร์เตอร์ $P_r$ ดังนั้น
$R_{dc}=\varepsilon (V_{dc}^{0})^{2}/P_r$ จุดสมดุลข้างต้นเมื่อโหลดเป็นกำลังคงที่ $P$
มีคำตอบจริงได้เฉพาะเมื่อดิสคริมิแนนต์ไม่เป็นลบ
\begin{equation}
  E_{dc}^{2}\ \ge\ 4R_{dc}P_{\max},\qquad P_{\max}=V_{\max}I_{\max},
  \label{eq:dcdisc}
\end{equation}
การเรียกร้องระยะเผื่อ $m$ ใน~\eqref{eq:dcdisc} จึงจำกัด $\varepsilon$ จากด้านบน ที่
$m=2$ ด้วย $V_{\max}=1.06\pu$, $I_{\max}=1.2\pu$ และ $P_{\mathrm{ref}}$ ที่งานนี้
สั่งไว้ ขอบเขตคือ $\varepsilon\lesssim0.115$ ค่าที่ประกาศใช้คือ $\varepsilon=0.10$
ซึ่งอยู่ภายในขอบเขตนั้น โดยระยะเผื่อที่ได้จริงบนการจัดรูปที่หัวข้อ~\ref{sec:sssa}
ตารางโมดัลใช้อยู่ในช่วง $2.07$--$2.15$ ตลอดทั้งสี่ลิงก์ และค่านี้ตรงกับค่าการกำกับ
สิบเปอร์เซ็นต์ตามปกติของลิงก์ DC ที่ป้อนผ่านภาคบูสต์หรือผ่านฟีดเดอร์ ค่านี้
\emph{ไม่ใช่}ค่าที่เลือกให้เหมาะกับขั้นเวลาของตัวอินทิเกรต ดูย่อหน้าเรื่องความแข็ง
ของระบบสมการด้านล่าง

\paragraph{$\tau_s$ มาจากเกณฑ์ตัวกรอง โดยมีขอบเขตประกบทั้งสองด้าน}
การเชิงเส้นแถว (3)--(4) รอบจุดที่สั่งไว้ขณะชอปเปอร์ปิดให้บล็อกขนาด $2\times2$
\begin{equation}
  A_{dc}=\begin{bmatrix}
  ...
  \label{eq:dcblock}
\end{equation}
โดย $K=\frac1C\bigl(\frac1{R_{dc}}-\frac{P_{ac}^{0}}{(V_{dc}^{0})^{2}}\bigr)$ และ
$a=\frac{P_{ac}^{0}}{C(V_{dc}^{0})^{2}}$ เงื่อนไขสองข้อจำกัด $\tau_s$ ได้โดย
\emph{ไม่}อ้างปริมาณใดที่รายงานผลไว้เลย ดีเทอร์มิแนนต์เป็นบวกต้องการ
$R_{dc}<(V_{dc}^{0})^{2}/P_{ac}^{0}$ ซึ่งเป็นเงื่อนไขโหลดกำลังคงที่ชุดเดียวกับที่
แบบจำลองพีชคณิตมีอยู่แล้ว การเพิ่มสเตตไม่ได้ผ่อนเงื่อนไขนี้ ส่วนเทรซเป็นลบต้องการ
\begin{equation}
  \tau_s<\frac{C(V_{dc}^{0})^{2}}{P_{ac}^{0}},
  \label{eq:dctau}
\end{equation}
ซึ่งเป็นเงื่อนไข\emph{ใหม่}: แหล่งจ่ายที่ตอบสนองช้าเกินไปยึดโหลดกำลังคงที่ไว้ไม่ได้
ระหว่างขอบเขตทั้งสองนั้น $\tau_s$ ถูกตรึงด้วยเกณฑ์แบนราบสูงสุด (maximally flat)
$\zeta=1/\sqrt2$ บนคู่ $L$--$C$ ซึ่งเป็นวิธีปกติที่ตัวกรองรูปนี้ถูกกำหนดสเปก และด้วย
$x=1/\tau_s$, $\zeta=(x-a)/(2\sqrt{Kx})$ ให้รูปปิด $\tau_s=1/y^2$,
$y=\zeta^{\star}\sqrt K+\sqrt{\zeta^{\star2}K+a}$ บนอุปกรณ์ชุดนี้ค่าที่ได้คือ
$\tau_s=5.00$~ms ทั้งสี่ตัว ค่าที่ประกาศไว้ในกรณีศึกษาจึงเป็น $5.00$~ms และอัตราการ
หน่วงที่เกิดขึ้นจริงอยู่ในช่วง $0.7072$--$0.7073$ ตลอดพิสัยภาระ $P_{ac}^{0}\in
[0.269,0.448]\pu$ ของทั้งสี่ลิงก์ --- ค่าคงตัวเวลาที่ประกาศไว้ค่าเดียวจึงใช้ได้ทั้ง
พิสัย และพารามิเตอร์นี้ยังคงเป็นค่าของชิ้นส่วน ไม่ใช่การจูนรายตัวแปลง ขอบเขตบน
\eqref{eq:dctau} หลวมกว่าค่าที่ใช้อย่างมาก คือ $223$--$372$~ms เทียบกับ $5.00$~ms
\textbf{ความเสื่อมถอย:} เมื่อ $\tau_s\to0$ คู่นี้แยกออกเป็นรากเร็วใกล้ $-1/\tau_s$
และรากช้าที่ $-K$ ซึ่งเป็นค่าลักษณะเฉพาะของแบบจำลองแหล่งจ่ายพีชคณิตพอดี ข้อนี้ยืนยัน
ด้วยตัวเลขและลดลงเชิงเส้นตาม $\tau_s$

\paragraph{สิ่งที่มาแทนขอบเขตกันการวิ่งหนี}
กับแหล่งจ่ายพีชคณิต ลิงก์เคยเป็นไปตาม $V_{dc}\le\max(V_{dc}(0),E_{dc})$ ทุกครั้งที่
$P_{ac}\ge0$ ด้วยการโต้แย้งเรื่องเครื่องหมายบนสมการแถวเดียว การโต้แย้งนั้น
\emph{ไม่รอด}เมื่อเพิ่มสเตตเข้ามา เพราะแถว (3) อ่าน $I_{dc}$ ซึ่งเป็นสเตตที่วิ่งไล่
ตามลักษณะสมบัติของตัวเองด้วยความหน่วง ไม่ใช่ฟังก์ชันของ $V_{dc}$ ณ ขณะนั้น ดังนั้น
$I_{dc}$ ยังเป็นบวกได้ขณะที่ $V_{dc}$ เกิน $E_{dc}$ ไปแล้ว และแหล่งจ่ายที่มีความ
เหนี่ยวนำก็\emph{แกว่งเกิน}จริง: ที่ $\zeta=1/\sqrt2$ ผลตอบสนองขั้นบันไดผ่านค่า
เป้าหมายไปครั้งหนึ่งราว $4.3$ เปอร์เซ็นต์ก่อนจะเข้าที่ สิ่งที่รอดคือ\emph{กลไก}
ไม่ใช่อสมการ --- แถว (4) ผลัก $I_{dc}$ ให้\emph{ลดลง}ทุกครั้งที่ $V_{dc}$ เกิน
$E_{dc}$ และ~\eqref{eq:dcblock} เสถียร คู่นี้จึงกลับเข้าที่ ที่จุดจ่ายที่สั่งไว้
$E_{dc}$ อยู่ในช่วง $1.0269$--$1.0448\pu$ ตลอดสี่ลิงก์ ซึ่งยังต่ำกว่าขีดเริ่มนำกระแส
ของชอปเปอร์ $V_{dc}^{\max}=1.10\pu$ แต่ตอนนี้นั่นเป็นข้อความเกี่ยวกับ\emph{จุดที่
เข้าที่แล้ว} ไม่ใช่ขอบเขตของการแกว่งเกิน

\paragraph{ความแข็งของระบบสมการที่เรื่องนี้เปิดเผย รายงานไว้ ไม่ได้ออกแบบหลบ}
รากช้าของ~\eqref{eq:dcblock} ซึ่งเป็นสเปกตรัมทั้งหมดของลิมิตแหล่งจ่ายพีชคณิตด้วย คือ
\begin{equation}
  \lambda_{dc}=-\frac{1}{C}\left(\frac{1}{R_{dc}}-\frac{P_{ac}^{0}}{(V_{dc}^{0})^{2}}\right),
  \label{eq:dclam}
\end{equation}
ซึ่งพจน์ที่สองคือความต้านทานส่วนเพิ่มค่าลบของโหลดกำลังคงที่ เสถียรภาพจึงต้องการ
$R_{dc}<(V_{dc}^{0})^{2}/P_{ac}^{0}$ --- เงื่อนไขที่การจัดรูปนี้สอดคล้องด้วยตัวประกอบ
กว้างมาก คือ $22$--$37$ เท่า ด้วย $C=C_{dc}=0.10$ และ $R_{dc}=0.10$ โมดนี้อยู่ระหว่าง
$-95.5$ และ $-97.3~\mathrm{s^{-1}}$ ค่าคงตัวเวลาราว $10$~ms ดังนั้น
$|\lambda_{dc}|\,\Delta t\approx4.8$ ที่ขั้นเฟเซอร์นาม $\Delta t=0.05\second$ กฎ
สี่เหลี่ยมคางหมูแบบอิมพลิซิตเป็น A-stable ที่นั่น แต่โมดนี้ถูกแทนด้วยความละเอียดไม่พอ
ที่ขั้นนาม และตัวควบคุมขั้นเวลาแบบปรับตัวต้องลด $\Delta t$ ลงในที่ที่มันถูกกระตุ้น
$\varepsilon$ \emph{ไม่}ถูกขยายเพื่อดึง $|\lambda_{dc}|\Delta t$ ให้เข้าใกล้หนึ่ง
เพราะการเลือกพารามิเตอร์ทางกายภาพให้เหมาะกับขั้นเวลาคือการย้อนวิศวกรรมมันจากความ
สะดวกเชิงตัวเลข ผลที่ซื่อสัตย์เป็นสิ่งตรงกันข้ามกับความสะดวก: แหล่งจ่ายที่
\emph{แข็งกว่า} ($\varepsilon$ ระดับ $0.01$ อย่างแบตเตอรี่ที่ขั้ว) แข็งขึ้นไปอีกและ
เป็นของการศึกษาแบบ EMT ไม่ใช่ของขั้นเฟเซอร์ $50$~ms นี่คือขีดจำกัดของการจำลองชนิดนี้
ที่ระบุไว้ตรงนี้ แทนที่จะถูกซ่อนด้วยอุดมคติ

\paragraph{ทำไมชอปเปอร์จึงไม่ใช่ของประดับ}
ทุกอย่างข้างต้นสมมติว่าตัวแปลง\emph{จ่ายกำลังออก} ตัวแปลงที่ถูกผลักให้\emph{ดูดกำลัง}
--- ถูกตรึงไว้ที่ขีดจำกัดกระแสขณะที่แรงดันและมุมของเกาะวิ่งหนีจากมัน --- เปลี่ยนพจน์
ภาระของแถว (3) ให้เป็นพจน์\emph{อัดประจุ} $+|P_{ac}|/V_{dc}$ จุดสมดุลของมันจึงเป็น
รากบนของ $V_{dc}^{2}-E_{dc}V_{dc}+R_{dc}P_{ac}=0$ ที่ $P_{ac}<0$ ซึ่งอยู่\emph{เหนือ}
$E_{dc}$ และสำหรับกำลังระดับที่ตัวจำกัดกระแสอิ่มตัวยอมให้ ก็อยู่เหนือ $V_{dc}^{\max}$
ด้วย ชอปเปอร์คือสิ่งที่หยุดมันไว้
กรณีนี้เกิดขึ้นจริงในแขนนโยบายควบคุมห้าแขนของงานนี้ บนสามแขนที่ไปถึงขอบฟ้า
แรงดันลิงก์สูงสุดของทั้งสี่ตัวแปลงคือ $1.0108$, $1.0224$ และ $1.0167\pu$ ชอปเปอร์ไม่นำ
กระแสเลย แต่บนแขนที่ตรึงสี่ตัวซึ่งล่ม IBR$_1$ ข้าม $V_{dc}^{\max}$ ที่ $t=20.4573\second$
และอยู่เหนือขีดนั้นต่อเนื่อง $94$ ตัวอย่างที่ยอมรับจนแขนจบ ยอดสูงสุด $1.1030\pu$ โดยมี
$P_{ac}\in[-0.925,-0.747]\pu$ และ $|i|\approx1.204\pu$ คือถูกจำกัดกระแสและดูดกำลัง
อยู่ทุกตัวอย่าง เมื่อจำกัดเฉพาะตัวอย่างของแขนเดียวกันที่ $P_{ac}\ge0$ ยอดสูงสุดคือ
$1.0482\pu$ ต่ำกว่าขีด รายงานนี้จึงระบุว่าชอปเปอร์\emph{ทำงานจริง}บนแขนนั้นและอยู่เฉย
บนสามแขนที่รอด ไม่ใช่ระบุว่าเป็นอุปกรณ์ป้องกันที่พิสูจน์แล้วว่าไม่จำเป็น การตัดสินว่ามัน
นำกระแสหรือไม่บนวิถีใดทำภายหลังจาก $\max_t V_{dc}$ เทียบ $V_{dc}^{\max}$ ซึ่งแม่นยำ
เพราะเงื่อนไขการนำกระแสขึ้นกับ $V_{dc}$ เพียงตัวเดียว
...
ลำดับที่ ส่วนจริงและส่วนจินตภาพของค่าลักษณะเฉพาะ $\lambda$ แยกเป็นคนละคอลัมน์
ความถี่โมดัล $f$ อัตราการหน่วง $\zeta$ และสเตตเด่น --- พิกัดเดียวที่มีค่าการมี
ส่วนร่วมสูงสุดเมื่อปรับบรรทัดฐานแล้ว คู่สังยุค (conjugate pair) พิมพ์เพียงแถวเดียว
ส่วนจินตภาพจึงเขียนด้วย $\pm$ และหมายถึงทั้งสองสมาชิกของคู่ ขณะที่โมดที่เป็นจำนวน
จริงล้วนพิมพ์ $0$ ที่คอลัมน์นั้นและ $\zeta=1$ แถวที่สองอันดับแรกของค่าการมีส่วน
ร่วมเข้าใกล้กันเกินจะจัดอันดับได้จะมีเครื่องหมายดาบและถูกนับรวมในเชิงอรรถของตาราง
เพื่อว่าชื่อที่พิมพ์จะไม่ถูกอ่านเป็นการระบุฝ่ายเดียวเมื่อมันไม่ใช่ ค่าการมีส่วนร่วม
เป็นข้อมูลเชิงรายงานและไม่เข้าเกตใด ตารางนี้คือตารางที่ตัวสร้างผลิตไว้เอง ไม่ใช่
สำเนาที่พิมพ์เรียงใหม่ จึงเป็นตารางเดียวกับที่รายงานฉบับภาษาอังกฤษใช้และตัวเลข
ตรงกันบิตต่อบิต
...
โมดที่มีความถี่สูงระดับร้อยเฮิรตซ์เป็นกลุ่มของกระแสตัวกรองและเครือข่าย คู่สังยุคสี่คู่
ที่ราว $15$~Hz ซึ่งมีส่วนจริงอยู่ระหว่าง $-95.6$ และ $-98.4~\mathrm{s}^{-1}$ คือ
ครอบครัวของลิงก์ DC ทั้งสี่ตัว และ
โมดที่ช้าที่สุดคือกลุ่มตัวอินทิเกรตของวงรอบแรงดัน ซึ่งเป็นตัวกำหนดขอบของอัตราการหน่วง
ในการจัดรูปนี้

\paragraph{โมดของลิงก์ DC เป็นการตรวจสอบแบบจำลอง ไม่ใช่ข้อสังเกตที่ได้มาฟรี}
คอนเวอร์เตอร์แต่ละตัวให้พิกัดด้าน DC มา\emph{สองตัว} คือ $V_{dc}$ และกระแสของแหล่งจ่าย
$I_{dc}$ และหัวข้อ~\ref{sec:ibr} ทำนายพฤติกรรมร่วมของทั้งคู่ไว้\emph{ก่อน}คำนวณ
สเปกตรัม สมการ~\eqref{eq:dcblock} เป็นบล็อกขนาด $2\times2$ ที่เป้าหมายการออกแบบแบบ
แบนราบสูงสุดของมันคือ $\zeta=1/\sqrt2$ ดังนั้นตารางจึง\emph{ต้อง}แสดงคู่สังยุคหนึ่งคู่
ต่อหนึ่งคอนเวอร์เตอร์ ที่อัตราการหน่วง $0.707$ และความถี่โมดัลใกล้ $15$~Hz --- และ
สี่คู่นั้นต้อง\emph{ต่างกัน} เพราะสิ่งเดียวที่ต่างกันระหว่าง $A_{dc}$ ของแต่ละตัวคือ
ภาระที่เข้ามาทาง $a=P_{ac}/(C V_{dc}^{2})$ และลิงก์ทั้งสี่รับกำลังไม่เท่ากัน
ตารางเป็นไปตามทุกข้อ: สี่คู่คือ
$-98.41\pm98.38\mathrm{j}$, $-96.46\pm96.33\mathrm{j}$,
$-96.35\pm96.21\mathrm{j}$ และ $-95.56\pm95.35\mathrm{j}$ ที่
$15.18$--$15.66$~Hz โดย $\zeta=0.7072$--$0.7079$ ไม่มีการปรับค่าใดเพื่อให้ได้ความ
สอดคล้องนี้: $\tau_s$ ถูกอนุมานจากเกณฑ์ตัวกรอง แล้วจึงคำนวณสเปกตรัมทีหลัง

\paragraph{วิธีอ่านค่าการมีส่วนร่วมของ DC และสิ่งที่มันบอกกับไม่บอก}
แต่ละคู่ในสี่คู่นั้นมีค่าการมีส่วนร่วม $0.500$ พอดีบน $V_{dc}$ ของคอนเวอร์เตอร์ตัวนั้น
และ $0.500$ พอดีบน $I_{dc}$ ของตัวเดียวกัน โดยพิกัดอื่นทุกตัวเป็น $0.000$ ทั้งสองครึ่ง
ของประโยคนั้นเป็นเรื่อง\emph{โครงสร้าง} และวัดบนระบบที่ประกอบแล้ว ไม่ใช่สิ่งที่สมมติขึ้น

\emph{การแบ่งครึ่งเท่ากัน}คือความสมมาตรของบล็อก $2\times2$ ที่มีคู่สังยุค: เมทริกซ์ย่อย
ด้าน DC ขนาด $8\times8$ ของแบบจำลองที่ประกอบแล้วเป็นบล็อกทแยงตามคอนเวอร์เตอร์
\emph{พอดี} โดยค่าที่มากที่สุดนอกบล็อก $2\times2$ ทั้งสี่ที่วัดได้เป็น $0$ แต่ละคู่จึง
อยู่ในปริภูมิยืนยงสองมิติ และพิกัดสองตัวในนั้นมีส่วนร่วมเท่ากัน

\emph{ศูนย์ที่พิกัดอื่น}คือการต่อกันแบบทางเดียว ซึ่งการเพิ่มสเตตของแหล่งจ่ายไม่ได้
เปลี่ยน การรบกวนพิกัด DC ตัวใดตัวหนึ่งไม่เปลี่ยนอนุพันธ์ของสเตตด้าน AC เลยแม้แต่ตัวเดียว
\begin{equation}
  \max_{i\,\in\,\mathrm{AC}}\ \max_{j\,\in\,\{V_{dc},I_{dc}\}}
  \left|\frac{\partial f_i}{\partial x_j}\right| = 0
  \quad\text{พอดี บนเมทริกซ์ที่ประกอบแล้วขนาด } 40\times40
  \label{eq:oneway}
\end{equation}
ขณะที่แถว DC อ่านสเตตด้าน AC จริง โดยค่าที่มากที่สุดคือ $33.4$ ครอบครัว DC จึงเป็น
ระบบย่อยแบบปิดที่ถูกขับโดยด้าน AC และไม่มีการมีส่วนร่วมของ DC ตัวใดไปถึง $i_d$ หรือ
$i_q$ เลย ผู้อ่านควรอ่าน $0.500/0.500$ ว่า ``โมดนี้คือวงจร DC ของคอนเวอร์เตอร์ตัวหนึ่ง
ซึ่งแบ่งกันระหว่างพิกัดสองตัวของมัน'' ไม่ใช่ ``วงจร DC ถูกต่อเข้ากับเครือข่าย''

\paragraph{สามรุ่นของพิกัดเดียวกัน}
ควรบันทึกไว้ว่าอะไรเปลี่ยนและอะไรไม่เปลี่ยน เพราะคอลัมน์การมีส่วนร่วมเคยให้ค่าอ่านมาแล้ว
สามแบบ รุ่นแรกพิมพ์สี่แถวที่\emph{เหมือนกันทุกแถว}ที่ $\lambda=-10.000000$ ด้วยค่าการมี
ส่วนร่วม $1.000$ นั่นคือแหล่งจ่ายอุดมคติ ซึ่งการป้อนไปหน้าของกำลังแบบพอดีตัดแถว DC ออก
จากทุกอย่าง ตรึงค่าลักษณะเฉพาะไว้ที่ $-1/T_{dc}$ ด้วยพหุคูณเท่าสี่ และทำให้ $V_{dc}$
คงที่เท่ากันหมดตลอดวิถี รุ่นที่สองพิมพ์\emph{รากจริงสี่ค่าที่ต่างกัน} โดยยังมีค่าการมี
ส่วนร่วม $1.000$ เพราะแหล่งจ่ายความต้านทานให้คอนเวอร์เตอร์แต่ละตัวมีพิกัด DC ตัวเดียว
พร้อมค่าลักษณะเฉพาะที่ขึ้นกับภาระของตัวเอง รุ่นปัจจุบันพิมพ์\emph{คู่สังยุคสี่คู่ที่
ต่างกัน} ด้วยการมีส่วนร่วมที่แบ่ง $0.500/0.500$ เพราะวงจรแหล่งจ่ายมีความเหนี่ยวนำ จึงมี
พิกัดที่สอง มีเพียงรุ่นแรกในสามรุ่นนั้นที่เป็นการทำให้เป็นอุดมคติ รุ่นที่สองกับรุ่นที่สาม
ต่างกันที่ $\tau_s$ เป็นศูนย์หรือไม่เท่านั้น และหัวข้อ~\ref{sec:ibr} แสดงว่ารุ่นที่สอง
เป็นลิมิตของรุ่นที่สาม

\paragraph{ข้อจำกัดของแบบจำลองที่อยู่เบื้องหลังสมการ~\eqref{eq:oneway}}
การต่อกันทางเดียวเป็นคุณสมบัติของแบบจำลองนี้ ไม่ใช่ของตัวแปลงจริง วงรอบชั้นในสร้างคำสั่ง
แรงดัน $v_{cd},v_{cq}$ จากความผิดพลาดของกระแสเพียงอย่างเดียว ไม่มีอะไรผูกคำสั่งนั้นกับ
แรงดัน DC ที่มีให้ใช้สังเคราะห์ ตัวแปลงสองระดับจริงผลิตได้ราว $m\,V_{dc}/2$ ต่อเฟส
ดังนั้นลิงก์ที่ตกจะตัดยอดคำสั่ง AC และการต่อกันจะกลายเป็นสองทาง สเตต DC จะปรากฏใน
แถว AC ความเป็นบล็อกสามเหลี่ยมจะแตก และการมีส่วนร่วมของ DC จะกระจายออก การเพิ่มขีดจำกัด
ดัชนีการมอดูเลตเป็นการเปลี่ยนสมการด้าน AC และไม่ได้ทำในงานนี้ จนกว่าจะทำ ลิงก์ DC ของ
งานนี้ตอบสนองต่อด้าน AC ได้แต่ย้อนไปกระทำกับมันไม่ได้ และไม่มีข้ออ้างใดในรายงานนี้
ที่อาศัยความสามารถนั้น
...
การคืนโหมดหลังต่อกลับ ($\NewRunNReselection$ ครั้ง) &
ที่ $\NewRunModeReselectionTime\second$ & หน่วย grid-forming สุดท้ายส่งคืน \\
สถานะการเลือกชุดใหม่หลังต่อกลับ & \NewRunReselection & ผ่านการตรวจกับตาราง SG-online \\
\bottomrule
\end{tabularx}
\end{table}
...
\paragraph{วงจรปิดสมบูรณ์: ตัวแปลงทุกตัวกลับสู่ grid-following}
เมื่อเครื่องกลับมาและทางลาดคำสั่ง C$_1$ จบลง ตัวกำกับจะทบทวนแผนที่โหมดเป็นครั้ง
สุดท้าย ที่ $t=\NewRunModeReselectionTime\second$ --- จุดตัวอย่างแรกหลังทางลาด
จบ โดย $\NewRunModeReselectionTime=\NewRunReclose+\NewRunHandback$ --- มันส่ง
หน่วย grid-forming ที่เหลืออยู่ตัวเดียวคือ IBR$_1$ กลับสู่ grid-following โดย
ผ่านการตรวจเป็นการปลดแบบหนึ่งขั้นที่ตรวจกับตาราง SG-online: การจัดรูป
all-grid-following ผ่านเกณฑ์ความหน่วงที่ประกาศไว้ด้วย $\zeta$ แย่สุด $0.156$
เทียบข้อกำหนด $0.05$ และเครื่องถือมุมอ้างอิงตั้งแต่การต่อกลับเป็นต้นไป จึงไม่จำเป็น
ต้องมีแหล่งสร้างแรงดันอีก สถานะปลายทางคือ all-grid-following โดยเครื่องเป็น
เจ้าของมุมอ้างอิงเพียงผู้เดียว --- เป็นการจัดรูปเดียวกับที่การส่งมอบแบบประสาน
(coordinated hand-back) ไปถึงโดยเส้นทางของมันเอง วงจรจึงปิดสมบูรณ์: เครื่องถูก
ตัด ตัวแปลงหนึ่งตัวรับหน้าที่สร้างแรงดัน หน่วยเสริมถูกเติมตามความรุนแรงและปลดเมื่อ
ความรุนแรงลดลง เครื่องกลับมา และตัวแปลงทุกตัวกลับสู่ grid-following
...
ต่อกลับ ก่อนการส่งคืนครั้งสุดท้ายตามย่อหน้าข้างบน ในทางกลับกัน การ\emph{ตรึง}
$\NewRunGfmMax$ ตัวไว้ตั้งแต่วินาทีที่ตัด SG ทำให้ระบบพังที่ $t=25.491\second$
ก่อนถึงขอบฟ้า ทั้งที่ชุดเดียวกันนั้นผ่านการรับรองเชิงเส้น นั่นคือเหตุผลที่ลำดับ
