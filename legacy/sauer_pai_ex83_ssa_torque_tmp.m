function result = sauer_pai_ex83_ssa_torque_tmp(flip)
if nargin<1; flip=false; end
...
resfun = @(z) residual(z,x0,y0,free_x,free_y,mp,Y,Tm,Vref,flip);
...
for i=1:nx
    ...
    Jxx(:,i)=(dae_f(xp,y,mp,Tm,Vref,flip)-dae_f(xm,y,mp,Tm,Vref,flip))/(2*h);
    Jyx(:,i)=(dae_g(xp,y,mp,Y)-dae_g(xm,y,mp,Y))/(2*h);
end
for j=1:ny
    yp=y; ym=y; yp(j)=yp(j)+h; ym(j)=ym(j)-h;
    Jxy(:,j)=(dae_f(x,yp,mp,Tm,Vref,flip)-dae_f(x,ym,mp,Tm,Vref,flip))/(2*h);
    Jyy(:,j)=(dae_g(x,yp,mp,Y)-dae_g(x,ym,mp,Y))/(2*h);
end
...
function res = residual(z,x0,y0,free_x,free_y,mp,Y,Tm0,Vref0,flip)
...
res=[dae_f(x,y,mp,Tm,Vref,flip); dae_g(x,y,mp,Y)];
end

function f = dae_f(x,y,mp,Tm,Vref,flip)
...
for k=1:ng
    ...
    f(ii+3)=omega;
    if flip; f(ii+4)=(Pe-Tm(k)-mp(k).D*omega)/mp(k).M; else; f(ii+4)=(Tm(k)-Pe-mp(k).D*omega)/mp(k).M; end
