function export_last_action(app, fig)
%EXPORT_LAST_ACTION Export button callback.

pfapp.set_busy(app, true);
pfapp.start_progress(app, fig, 'Exporting results ...', 'Writing files under the output folder.');
cleanup = onCleanup(@() pfapp.set_busy(app, false));
try
    output_dir = fullfile(pwd, 'output');
    if ~isempty(app.last_result)
        prefix = pfapp.make_safe_name(sprintf('%s_%s', app.last_result.system_name, app.last_result.method));
        paths = pf_export_results(app.last_result, output_dir, prefix, struct('export_figures', true));
    ...
    elseif ~isempty(app.last_cpf)
        prefix = pfapp.make_safe_name(sprintf('%s_%s', app.last_cpf.system_name, app.last_cpf.method));
        paths = pf_export_cpf_results(app.last_cpf, output_dir, prefix, struct('export_figures', true));
        pfapp.append_log(app, sprintf('Exported CPF: %s', paths.summary_txt));
    elseif ~isempty(app.last_opf)
        prefix = pfapp.make_safe_name(sprintf('%s_%s', app.last_opf.system_name, app.last_opf.method));
