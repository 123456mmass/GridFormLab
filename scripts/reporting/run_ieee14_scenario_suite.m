...
for k = 1:numel(rows)
    ...
    % The horizon the METRICS are measured against is the realized one, not the
    % runner's default: a scenario may declare its own t_end in opt_overrides
    % (sg_fault_cycle160 does), and passing opts.t_end here would report that arm
    % as exceeding its horizon while measuring horizon_fraction against the wrong
    % denominator.
    m = ieee14_arm_metrics(r,a,op.t_end);
...
% --- 4. The reference-owning converter is lost, then the machine returns ---
...
% moved.
%
% sg_on = 90 s, so this arm asks TWO questions in sequence: does the framework
% recover the angle reference by itself after the owner is lost, and can the
% machine then resynchronise onto an island that has permanently lost a
% converter? Recovery alone was the original question, but an arm that never
% recloses cannot show the hand-back, which is the second half of the claim --
% and every other arm in the suite shows it. The outage is at 60 s and the offer
% at 90 s, leaving 30 s for the post-outage island to settle before the
% synchronism gates are asked to pass.
%
% There is no IBR reclose: the tripped converter stays out for the rest of the
% run. The machine comes back; the converter does not.
rows(4) = struct( ...
    'id','former_outage', ...
    'label',['SG trip, outage of the converter that owns the island angle ' ...
        'reference, then SG reclose'], ...
    'short_label','former outage', ...
    'scenario_fn',@eecon49_scenario, ...
    'event_overrides',struct( ...
        'event_profile','sg_trip_then_former_outage', ...
        'sg_trip',20,'ibr_trip',60,'ibr_trip_target','reference_owner', ...
        'sg_on',90,'fault_bus',[],'Zf',[],'fault_on',[],'fault_clear',[], ...
        'load_step',[],'load_step_factor',[], ...
        'line_trip',[],'line_from_bus',[],'line_to_bus',[],'restore_time',[]), ...
    'opt_overrides',struct(), ...
    'expectation','TRAJECTORY_THEN_ANY', ...
    ...
    'question',['When the converter holding the angle reference is lost, does the ' ...
        ...
        'authenticated configuration survives -- and can the machine then ' ...
        'resynchronise onto an island that has permanently lost a converter?'], ...
    'defining_event','ibr_trip');

% --- 5. The full cycle on one 160 s horizon --------------------------------
% Island, the converter that replaces the machine as the angle reference, a
% bolted fault at bus 9 ridden through on that island, then the machine back.
% This is the arm the presentation walks through stage by stage, which is why it
% is spaced rather than compressed: 20 s of normal operation, 40 s of island
% before the fault, 40 s of post-fault island before the machine is offered, and
% 60 s after the offer for the reclose and the hand-back to settle.
%
% Stage 2 -- "a converter takes the reference" -- is NOT a scheduled event and is
% not in event_overrides. It is the supervisor's own decision, taken about
% T_d_on = 0.10 s after the trip, and reading it out of the trajectory rather
% than scheduling it is the point: the framework chooses which converter forms.
%
% t_end = 160 s is declared in opt_overrides, not in the runner's t_end. The
% other four arms keep the 120 s horizon their caches were written at, so this
% row cannot invalidate them.
rows(5) = struct( ...
    'id','sg_fault_cycle160', ...
    'label',['SG trip, a converter takes the island angle reference, bolted ' ...
        'fault at bus 9, then SG reclose, over 160 s'], ...
    'short_label','full cycle 160 s', ...
    'scenario_fn',@eecon49_scenario, ...
    'event_overrides',struct( ...
        'event_profile','sg_fault_cycle', ...
        ...
        'load_step',[],'load_step_factor',[], ...
        'line_trip',[],'line_from_bus',[],'line_to_bus',[],'restore_time',[]), ...
    'opt_overrides',struct('t_end',160), ...
    'expectation','TRAJECTORY_THEN_ANY', ...
    'expected_failure_id','', ...
    'artifact','sg_fault_cycle160.mat', ...
    'color',[0.30 0.30 0.30], ...
    'line_style','-', ...
    'classification','PROJECT_RESULT', ...
    'question',['Over one 160 s horizon: does a converter take the angle ' ...
        'reference when the machine leaves, does the island ride a bolted ' ...
        'fault through, and does the machine come back and take the reference ' ...
        'again?'], ...
    'defining_event','fault_clear');
end
...
%
%   anti_windup_blend = 1e-3 is set for EVERY scenario here, and it is the one
%   option in this set the delivered chronology runner does not carry. It must be
%   declared, not slipped in, because it changes the model: the IBR anti-windup
%   switch engages continuously over a band of 0.1 % of Imax instead of jumping.
%   The reason is a measured sliding mode. With the hard switch, an island whose
%   converter reference sits exactly on r = Imax has NO self-consistent branch:
%   solved with the hold on, the solution lands where the limiter is inactive;
%   solved with it off, it lands where the limiter is active. The branch
%   classification then 2-cycles indefinitely and the root of either pure branch
%   misses the true switched residual by 1.9e-06, so the step controller runs to
%   its floor and the run stops inside the fault window at t = 50.0833 s. That is
%   the defect recorded in
%   docs/project/defects/2026-09-04-limiter-antiwindup-sliding-mode.md, where the
%   falsified alternatives are listed: backward-Euler order reduction, FD
%   perturbation scaling, per-column Jacobian construction and floors down to
%   0.05/2^18 all reproduce the same stop to the digit.
%
%   NOTHING about the acceptance authority changes: newton_tol = 1e-8,
%   kcl_tol = 1e-6, the Richardson LTE test, dt_min and reject_limit are the
%   delivered values. Outside the band the blended and hard forms are identical
%   expressions, and the option defaults to 0 (hard switch) everywhere it is not
%   set -- verified by re-running the delivered 250 s chronology, which reproduces
%   reclose 159.252 s, 3679 samples, 161 rejected steps, max accepted KCL
%   residual 9.9721e-9 and terminal 60.000001 Hz exactly.
