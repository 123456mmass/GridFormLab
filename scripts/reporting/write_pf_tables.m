function write_pf_tables(pf,outdir,lim,bus_role)
%WRITE_PF_TABLES Publish bus/line power-flow tables (CSV + LaTeX).
%   WRITE_PF_TABLES(PF,OUTDIR) writes the standard tables.  LIM is the
...
% Type label: prefer the case bus_role so GFL/GFM inverter buses are visible.
role=strings(height(bus),1);
for k=1:height(bus)
    if ~isempty(bus_role) && numel(bus_role)==height(bus)
        role(k)=string(bus_role(k));
    else
        role(k)=string(type_name(bus.type(k)));
    end
end
bus.role=role;
...
for k=1:height(bus)
    ...
    fprintf(fid,'%d & %s & %.4f & %.4f & %.4f & %.4f & %.4f & %.4f & %s & %s & %s \\\\\n', ...
        bus.bus(k),bus.role(k),v(1),v(2),v(3),v(4),v(5),v(6), ...
...
% Fixed two decimals so the limit columns line up with each other; a dash
% marks a bus with no generator (no finite limit).
if ~isfinite(x) || isnan(x), s='--'; else, s=sprintf('%.2f',x); end
end
