...
%
%   Gate semantics: PGAz missing/not-run/plateau-not-reached => FAIL. The
%   plateau-PGAz-vs-Ours comparison uses a TIGHT tolerance (0.05 deg COI)
%   justified a priori by the PSAT-Ours converged-trapezoidal baseline
%   (~0.01 deg at dt=0.01): two converged trapezoidal solutions of the SAME
%   problem at the SAME dt must agree to <0.05 deg. If PGAz exceeds this, the
%   gate FAILS (root cause traced, not tuned).
...
gates.tol = TOL;
gates.all_gates_pass = all([gates.contract_ybus_pgaz, gates.contract_ybus_psat, ...
    gates.gen_mapping_psat, gates.gen_mapping_pgaz, gates.comparison_grid_valid, ...
    gates.event_grid_valid, gates.sample_alignment_psat, gates.sample_alignment_pgaz, ...
    ~gates.extrapolation_used, gates.psat_execution, gates.pgaz_execution, ...
    gates.pgaz_plateau, gates.ours_convergence, gates.psat_comparison, gates.pgaz_comparison]);
