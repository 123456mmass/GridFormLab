...
opt.t_end = 1.20;
% sg_on is REQUIRED by this profile: sg_trip_then_former_outage arms sg_reclose,
% so the schedule refuses an arm without a reclose offer (ibr_event_schedule.m:
% has_sg_reclose -> required field). The offer is placed inside the horizon at
% 1.10 to satisfy sg_trip < ibr_trip < sg_on <= t_end.
%
% The close itself does NOT land here, and that is the contract rather than a
% shortfall: T_sg_min_off_s = 0.5 s (case_ieee14_1sg_4ibr_auto_vsg.m:133) holds
% the breaker out until t >= 1.40, past this horizon. Extending the horizon to
% reach it would double the arm's runtime to re-test what the 150 s scenario in
% scripts/reporting/run_ieee14_scenario_suite.m already measures (reclose SUCCESS
% at 90.3374 s). The subject here stays the outage commit.
